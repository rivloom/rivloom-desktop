...
Keep all original user constraints in force. Sensitive credential reads, subagents and skill loading remain prohibited. An engine permission request must still be handled through its permission interface; never claim that a request was approved merely because you said so. Report only actions actually completed. Respond in the user's language.`;
}
