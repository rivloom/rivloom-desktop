...
let bridge: { url: string; token: string; context: ((root: string) => { url: string; token: string }) | null } | null = null;
export function knowledgeEngineConfig(root?: string) {
  return bridge ? { url: bridge.url, token: bridge.token, context: root ? bridge.context?.(root) : undefined,
...
export async function startKnowledgeBridge(tools: () => KnowledgeTools | null, context?: (root: string, body: unknown) => unknown,
  ...
  const server: Server = createServer(async (req, res) => {
    ...
    const scoped = req.url === '/context' || req.url === '/history';
    ...
    if (req.method !== 'POST' || !['/tools', '/context', '/history'].includes(req.url || '') || req.headers.origin ||
      scoped && (!scope || !(req.url === '/history' ? history : context)) ||
  ...
  bridge = { url: `http://127.0.0.1:${address.port}/tools`, token: secret, context: context ? root => {
    const key = resolve(root);
    if (!scopes.has(key)) scopes.set(key, randomBytes(32).toString('hex'));
    return { url: `http://127.0.0.1:${address.port}/context`, token: scopes.get(key)! };
  } : null };
