...
**当前正式版：Linux x64 0.1.22（2026-09-26）。** 源码 [9b3f07f759b278bbed8dbcd910ea12889188bb0a](https://github.com/rivloom/rivloom-desktop/commit/9b3f07f759b278bbed8dbcd910ea12889188bb0a)；[原生构建](https://github.com/rivloom/rivloom-desktop/actions/runs/36217814357)与[独立发行](https://github.com/rivloom/rivloom-desktop/actions/runs/36229729384)、公开 curl/wget 各自完整下载及校验、WSL 全树/启动/重启/SIGTERM、官网双语命令和 R2 保留检查通过（保留 81、暂缓 0、可删除 0、删除 0 个）。核心固定 `1.18.31-rivloom.9b07cf442a7e`，SDK/plugin 1.18.31，Node.js 24.19.0。继续手动升级，SHA256 不等于数字签名；ARM64 未发布。文件与范围见[Linux 0.1.22 说明](releases/linux-0.1.22.md)及[整版验收](releases/0.1.22-verification.md)。本轮没有新增实体 Linux 多机、真实模型或完整升级矩阵验收，命令行节点不代表 WebKitGTK 桌面视觉已验收。
...
本文介绍随包 Linux x64 节点。安装版本用 `rivloom --version` 核对；对外发行、下载地址及校验值以[官网下载页](https://rivloom.com/download/)和对应发行记录为准。0.1.22 功能与边界见[版本说明](releases/linux-0.1.22.md)。核心固定 `9b07cf4`，运行要求为内核至少 4.18、glibc 至少 **2.30**、libstdc++ 至少 6.0.25；ARM64 暂不发布。
...
从 [官网下载页](https://rivloom.com/download/) 选择 Linux x86_64，并复制页面给出的完整 curl / wget 命令和该版本的 SHA256；也可从 [GitHub 发行列表](https://github.com/rivloom/rivloom-desktop/releases)选择相同版本的 Linux 条目。校验通过后解压，无需 `sudo`，也不执行下载下来的安装脚本。

文件名为 `Rivloom_<版本>_linux_x64.tar.gz`。解压后的顶层目录是 `rivloom/`，包含启动入口 `bin/rivloom`、固定 Node.js、OpenCode 引擎、应用和依赖许可。0.1.18 首版内置官方引擎；自 0.1.19 起，自有 ELF 和来源记录放入 `app/vendor/rivloom-opencode/linux-x64/<commit12>/`。包内带运行时，不要求额外安装 Node.js 或 npm。
