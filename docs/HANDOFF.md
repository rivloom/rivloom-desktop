...
6. **M3.3 策略化跨节点执行：第三段完成。** 设备信任、本机执行能力和 AI 操作审批已分层。受信任务自动接收，能力关闭时等待；开启后复用本机项目和模型。任务锁定“请求批准 / 帮我批准 / 允许任何操作”，经官方 OpenCode 会话权限接口执行。归属 Brain 收到单调状态和最小人工介入快照，并已能通过认证加密通道批准/拒绝具体操作、回答 AI 和停止执行。真实 Release 已验证一个会话在人工介入点被远程停止，另一个会话经远程一次性批准后生成预期文件。控制绑定任务和执行序号并持久化去重；本机项目、模型、任务 ID、审批 metadata 或凭据不回传。补充要求、差异/产物、验收和完整人员角色映射仍待做。
...
- M3 Release WebView2：23 项通过；真实第二节点完成发现/配对、可信任务自动接收、能力关闭时不启动、发起方界面回答/拒绝/停止、真实 OpenCode 远程停止与一次性批准后继续、文件生成、有序状态、远端隐私边界、同项目并发拒绝及设备撤销。
...
- Win10 `192.168.5.18` 与 Win11 `192.168.5.20` 的受信任务自动接收、三种 AI 审批、远程批准/回答/停止、真实 OpenCode 执行、撤销信任停止任务及异常进程结束/断网兜底物理复测；补充要求、差异/产物、角色映射和两位真人验收仍未完成。
- ChatGPT 登录。
...
下一步在已完成的远程批准/拒绝、回答和停止之上接补充要求，再接差异/产物及验收，并明确跨设备人员角色映射。现有和新增控制继续校验任务路由、设备信任、执行序号和任务版本。业务应用和引擎仍只监听 loopback；OpenCode 永远不直接暴露到局域网或公网。
...
> 请先阅读 `docs/HANDOFF.md`、`docs/PROGRESS.md`、`docs/adr/0001-self-discovering-brain-network.md`、`docs/adr/0002-configurable-node-invocation-policy.md`、`docs/adr/0003-trust-and-ai-approval.md` 和实施计划。先核对仓库状态，不重做 M0/M1/M2 或 M3 已完成的底层协议，不使用过期 OpenCode Go，不修改旧项目。设备信任、本机执行能力与 AI 审批已经分层；受信任务自动接收，三种审批模式经官方 OpenCode 会话权限传入；远程批准/拒绝、回答和停止已经通过真实 Release。下一步接补充要求、差异、产物、验收和跨设备人员角色映射。
