...
4. **M3.1 节点身份与自发现：双向发现通过，生命周期复测中。** 每个实例有 DPAPI 保护的稳定 Ed25519 身份；标准 `_rivloom._tcp.local` 与 LAN UDP 回退只产生候选地址，随后执行随机挑战签名校验，桌面节点页明确显示未配对状态。原包在 Win10 `192.168.5.18` 与 Win11 `192.168.5.20` 出现单向 mDNS；UDP 回退包已在原设备确认双向发现。现场随后发现关闭节点在原 120 秒窗口内仍显示在线，已改为约 10 秒心跳、30 秒离线、120 秒移除，等待原设备验证关闭与恢复。它不代表授权或 Brain 任务协作已经完成。
...
- 大小：71,010,576 字节
- SHA-256：`3fb676844d87998cc49174fbe50a49d215b24c05fa40b3607e5224cb1b2ecc5b`
...
- 新生命周期版本在 Win10 `192.168.5.18` 与 Win11 `192.168.5.20` 上的离线、移除、恢复及身份稳定复测；之后仍有设备配对/撤销、加密业务通信、单 Brain 分布式任务、最小跨 Brain 委派，以及两位真人验收。
- ChatGPT 登录。
