...
更新时间：2026-09-02。工作目录：`C:\project\rivloom-opencode`。
...
- 当前开发里程碑仍是 M3.3；单人双机核心闭环已经人工验收，下一步先修复安装器防火墙可靠性并补测停止/自动审批/撤销，再明确两真人角色，之后才讨论 M3.4。
...
上一版已验证的内测安装包哈希已被本轮重建产物取代。当前开发机产物：

- 路径：`src-tauri/target/release/bundle/nsis/Rivloom_0.1.0_x64-setup.exe`
- 大小：71,070,646 字节
- SHA-256：`d688e2d9a2f978fc7bde08fa09c88ffc17ab3bc86f607e3947bd9e92b281ef01`
- 状态：未签名内测包；本轮构建通过，但尚未重新执行隔离安装、随包引擎启动、进程清理和卸载烟雾测试。

对应 `src-tauri/target/release/Rivloom.exe` 为 10,264,576 字节，SHA-256 `9da6c7b764f4cef3a33a02508ff4fbb414fba84c3721a47295d31bdf1f5c24cd`。再次发给其他设备前需复测当前 NSIS；正式发行还需代码签名和干净机矩阵。
...
- 上一版 NSIS 的安装/启动/卸载烟雾测试通过；当前 NSIS 已重建但尚未复测。
- M3 Release WebView2：上一完整报告 23 项通过；真实第二节点完成发现/配对、可信任务自动接收、能力关闭时不启动、发起方界面回答/拒绝/停止、真实 OpenCode 远程停止与一次性批准后继续、文件生成、有序状态、远端隐私边界、同项目并发拒绝及设备撤销。本轮实际 Release 又完成新增官方差异卡片、补充要求和验收的加密往返，随后免费模型 10 分钟无响应，未生成新的全组通过报告。
...
- 本地：TypeScript 检查与生产构建通过；16 项测试通过；Cargo fmt/clippy 通过；生产依赖离线 audit 为 0 个已知漏洞。
...
- 物理双机仍未覆盖远程停止、“帮我批准 / 允许任何操作”、执行中撤销信任、异常进程结束/断网兜底和两位真人角色映射。
- 安装器没有可靠管理防火墙：`5.33` 的随包 `node.exe` 曾存在启用但 Action=Block 的 Private TCP/UDP 规则，手工改为 Allow 后发现恢复。下一次开发必须用仅限 Private/LocalSubnet 的命名规则解决安装、升级、诊断和卸载。
- ChatGPT 登录。
...
下一步先修复并验证安装器的 Windows 防火墙规则，再补测停止、另外两种审批、撤销信任和重启保持，并明确跨设备人员角色映射。全部控制继续校验任务路由、设备信任、唯一控制 ID 和执行序号；业务应用和引擎仍只监听 loopback，OpenCode 永远不直接暴露到局域网或公网。
...
> 继续开发 `C:\project\rivloom-opencode`。先阅读 `docs/HANDOFF.md`、`docs/MILESTONES.md`、`docs/PROGRESS.md`、`docs/VERIFICATION.md` 和三份 M3 ADR，核对 Git 后继续，不重做已完成协议。当前 `main` 已实现 M3.3；用户已在 `192.168.5.20` 与 `192.168.5.33` 用真实 DeepSeek 完成单人双机文件生成、远程一次性批准、补充要求同会话续跑、本地核对和最终验收。测试前因 `5.33` 随包 `node.exe` 的 Private TCP/UDP 规则 Action=Block 导致单向发现，手工改 Allow 后恢复。下一步先为 NSIS/客户端实现仅限 Private/LocalSubnet 的命名防火墙规则，覆盖安装、升级、诊断和卸载并复测双机发现；再补测远程停止、另外两种审批、执行中撤销信任和两真人角色映射。M3.4 继续暂停，不修改或 fork OpenCode，不要求 Git，不用文件快照或哈希。
