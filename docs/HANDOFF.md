...
4. **M3.1 节点身份与自发现：物理双机复测中。** 每个实例有 DPAPI 保护的稳定 Ed25519 身份；标准 `_rivloom._tcp.local` 与 LAN UDP 回退只产生候选地址，随后执行随机挑战签名校验，桌面节点页明确显示未配对状态。原包在 Win10 `192.168.5.18` 与 Win11 `192.168.5.20` 出现单向 mDNS；TCP 握手端口可达。UDP 查询/单播回复回退已经实现并通过强制关闭 mDNS 的同机双实例测试，等待新包在原两台机器复测。它不代表授权或 Brain 任务协作已经完成。
...
- 大小：71,011,083 字节
- SHA-256：`ddd7453081de8d4e31a350e3459440ef74d92ff75786ed8e1beb07e5ef94cea4`
...
- 新回退版本在 Win10 `192.168.5.18` 与 Win11 `192.168.5.20` 上的双向发现复测；之后仍有设备配对/撤销、加密业务通信、单 Brain 分布式任务、最小跨 Brain 委派，以及两位真人验收。
- ChatGPT 登录。
