...
- M0、M3.1、M3.2 已由用户确认完成。
- 用户已经创建 DeepSeek Key；下一步在 Rivloom 本机完成 M1 的真实连接和固定编程任务验证，Key 不进入聊天或仓库。
...
2. **M1 模型设置：无 Key 范围完成，真实验证待做。** 桌面端已有“模型与额度”页；用户已创建 DeepSeek Key，下一步只在 Rivloom 本机保存并执行真实连接和固定编程任务。操作审计不保存 Key 或模型回复。
...
6. **M3.3 策略化跨节点执行：第一段完成。** 执行节点一次配置项目、模型和自动/有限/每项确认策略。匹配任务自动接受，只绑定一个本机业务任务并启动官方 OpenCode；归属 Brain 收到单调执行状态和最终摘要，不收到本机项目、模型、任务 ID 或凭据。实际 Release WebView2 已由真实第二节点发任务，并验证随包 OpenCode 在隔离 Git 项目生成预期文件。远程审批、补充、停止、差异/产物和验收仍待做。
...
- Win10 `192.168.5.18` 与 Win11 `192.168.5.20` 的策略化自动调用、真实 OpenCode 执行、撤销信任停止任务及异常进程结束/断网兜底物理复测；远程审批/补充/停止、差异/产物、角色映射和两位真人验收仍未完成。
- ChatGPT 登录。
...
若用户暂不提供 Key，下一步在已完成的策略化自动执行和有序状态之上实现跨设备角色与控制消息：先接审批/拒绝、回答、补充要求和停止，再接差异/产物及验收。所有控制必须校验任务路由、角色、事件序号和任务版本。业务应用和引擎仍只监听 loopback；OpenCode 永远不直接暴露到局域网或公网。
...
> 请先阅读 `docs/HANDOFF.md`、`docs/PROGRESS.md`、`docs/adr/0001-self-discovering-brain-network.md`、`docs/adr/0002-configurable-node-invocation-policy.md` 和实施计划。先核对仓库状态，不重做 M0/M1/M2 或 M3 已完成的底层协议，不使用过期 OpenCode Go，不修改旧项目。策略化自动/有限/每项确认、本机真实 OpenCode 执行和单调状态回传已完成；下一步接跨设备角色、审批/回答/补充/停止，再接差异、产物和验收。
