...
4. **M3.1 节点身份与自发现：双向发现通过，生命周期复测中。** 每个实例有 DPAPI 保护的稳定 Ed25519 身份；标准 `_rivloom._tcp.local` 与 LAN UDP 回退只产生候选地址，随后执行随机挑战签名校验，桌面节点页明确显示未配对状态。原包在 Win10 `192.168.5.18` 与 Win11 `192.168.5.20` 出现单向 mDNS；UDP 回退包已在原设备确认双向发现。现场随后发现关闭节点在原 120 秒窗口内仍显示在线；现已增加正常关闭的签名离线通知，异常断线改为每 5 秒心跳、约 15 秒离线、约 30 秒移除，等待原设备验证正常关闭、异常断线与恢复。它不代表授权或 Brain 任务协作已经完成。
5. **M3.2 配对第一切片：同机真实验证完成。** 签名请求和确认绑定节点 ID、指纹、来源地址、随机会话与时间窗；两台设备显示相同六位短码和完整指纹，只有双方分别确认才持久授信。重放请求拒绝、重启保持、取消和双方撤销已通过自动化与 Release WebView2；物理双机仍待用户复测，业务通道仍关闭。
6. **桌面直接启动：已实现。** 首次打开不再创建“工作区”或询问显示名称、用户名、密码；Tauri 启动期随机令牌自动建立/恢复本机操作者，直接进入任务界面并开始发现。内部 Web 调试仍保留显式登录。
7. 独立账号、一次性邀请、发起/接受/审批/验收角色已由服务端协议测试验证。它是后续业务权限基础，当前还没有跨节点传输。
...
- 大小：71,044,500 字节
- SHA-256：`d186d258a434a547692a52250e7e6170e55649a2f5ad4ef77f7051e0eda21dcf`
...
- M3.1/M3.2 Release WebView2：9 项通过；真实第二节点完成发现、同码、单方不授信、双方授信和撤销。自动化分别强制验证 mDNS、LAN UDP、重放拒绝、重启保持和撤销。
- 本地：TypeScript 检查与生产构建通过；12 项测试通过；Cargo fmt/clippy 通过；生产依赖离线 audit 为 0 个已知漏洞。
...
- 新版本在 Win10 `192.168.5.18` 与 Win11 `192.168.5.20` 上的生命周期、身份稳定、双端配对、重启保持、撤销及重新配对复测；之后仍有认证加密业务通信、单 Brain 分布式任务、最小跨 Brain 委派，以及两位真人验收。
- ChatGPT 登录。
...
若用户暂不提供 Key，先在两台 Windows 设备完成生命周期和 M3.2 双端配对/撤销复测，再设计认证加密节点业务通道。业务应用和引擎仍只监听 loopback；目前局域网只开放签名公开身份和配对控制端点，OpenCode 永远不直接暴露到局域网或公网。
...
> 请先阅读 `docs/HANDOFF.md`、`docs/PROGRESS.md`、`docs/adr/0001-self-discovering-brain-network.md` 和实施计划。先核对仓库状态，不重做 M0/M1/M2/M3.1，不使用过期 OpenCode Go，不修改旧项目。若我提供 DeepSeek Key，只指导我在客户端本机输入；否则先做 M3.1 物理双机验证，再实现 M3.2 双方确认配对、信任与撤销，不做单主机邀请式协作。
