...
**2026-09-15 · 发布阻塞：** 0.1.15 源码 `97ab793` 已推送，目标提交全部 CI、原生打包和隔离安装/启动/重启/卸载通过。GitHub 因账户付款或 Actions 支出限额拒绝启动发布 job，运行 [34946471004](https://github.com/rivloom/rivloom-desktop/actions/runs/34946471004)，候选 artifact `10387819109`。用户处理计费后继续失败步骤；无需再次申请版本/提交/升级授权。正式入口仍为已验证的 0.1.14，0.1.15 尚未创建 Release、上传 R2、签名提升或部署官网，R2 保留检查尚未执行，不得记作收尾完成。详情见 [验证记录](VERIFICATION.md) 顶部。

**2026-09-15 · 0.1.15 CI 修复待重跑：** 首个提交 `6e43f3b` 已推送；隔离服务依赖定位导致官方引擎 CI 失败，候选门禁未发行。修复依赖定位，并用确定性回归修复异步协作回执的序列竞争。416 项逻辑、13 项协议、69 项 CI 自检、CI 原入口知识库 6 组、额外连续 24 次远端读取及构建通过。继续推送修复并完成同提交 CI、安装、公开下载/验签、官网和 R2 收尾；正式入口仍为 0.1.14，不需要重复申请已给出的提交升级授权。见 [验证记录](VERIFICATION.md)。

**2026-09-15 · 0.1.15 发布进行中：** 用户已授权补丁版本加一、提交与升级，共享 Skills/Wiki 和开源准备修复一并进入本版。版本与发布说明已同步；接下来按既有 main 自动发布流程完成同提交 CI、隔离安装、R2/签名更新、官网实际部署和 R2 保留检查。仓库公开另行处理。下面未获发行授权的文字属于此前阶段，不应再要求重复确认。

**开源准备交接（2026-09-15，工作区未提交）：** 许可、贡献和安全说明、官方发布来源保护及策略回归已完成，414 项逻辑、68 项 CI 自检和独立源码构建通过。全部 399 个 artifacts、15 份 SHA256SUMS、15 个 Release 安装器及 102 份日志已完成相应完整性核验；安装包解包和当前/历史二进制可读字符串扫描已补齐，命中均经复核为上游样例、公开标识或误报，未发现新增真实凭据。**用户已明确确认在账号内撤销 ChatGPT 分享，当前四处引用已移除。** 此结论来源是用户确认，匿名请求超时和浏览器失败不算独立验证通过。保留原仓库、CI/CD 和 Git 历史，不再按旧方案要求重复确认、迁移或重写。详情及剩余公开步骤见 [开源准备](OPEN-SOURCE-READINESS.md)。四仓库分工、main 自动发布和公开时开启私密漏洞报告的决定不变；本轮未公开、提交/推送、改权限、运行安装器、调用真实模型或部署官网。正式版仍为 0.1.14，Skills/Wiki 保留未发布工作区状态。

**最新开发（2026-09-14，未发布）：** 共享 Skills 与分层 Wiki 记忆第一版已在工作区实现。侧栏“技能与记忆”可登记 Skill、手动分享/撤销、查看 Brain 目录及最新版；A→Brain→B 中继与官方引擎实际执行已通过。记忆默认本机、手动分享给 Brain，支持分类、项目范围、历史、两级 AGENTS.md/agent.md 和按需加载；自动整理目前是分类索引及重复标记，独立 AI 语义整理尚未加入。413 项逻辑、13 项协议、3 项引擎、6 组三节点服务、65 项 CI 自检与 UI/双语/生产构建通过。详见 [知识库](KNOWLEDGE-LIBRARY.md) 和 [验证记录](VERIFICATION.md)。`npm run preview:knowledge` 打开合成验收；`npm run test:knowledge` 复测真实隔离服务。未提交、推送、打包、安装或发布，正式版仍为 0.1.14；不要重复开发已完成项。真实账号由用户操作，真实额度调用仍须先确认。
...
**2026-09-13 最新开发交接：** 会话内容搜索与匹配定位已完成，待用户次日验收。历史轮次、旧执行尝试、长消息和排队消息可搜索；保留目录/状态/来源筛选、草稿与结果复制。已完成 393 项逻辑、14 组搜索界面和 13 组输入框回归；正式版仍为 0.1.13，本轮未提交、推送、发布或安装。使用说明和本机预览入口见 [CONVERSATION-SEARCH](CONVERSATION-SEARCH.md)，证据见 [VERIFICATION](VERIFICATION.md) 顶部。保留所有既有 R2 文档改动，后续发布继续执行已规定的保留检查。

**2026-09-11 最新交接：** 0.1.13 已发布且官网已同步，桌面 `14ee3931b3e266b50717684d8510f0500c4110c9`，官网 `8f0a1334d3753d83281ea7ae57f0fce992673d96`。后续发布须遵守根目录 `AGENTS.md` 与 [R2 保留与清理](R2-RETENTION.md)，保留最近 3 个正式版本、近 30 天的包及当前引用；按并集核对并记录结果。当前是发布执行者的必做检查，尚无自动清理代码，本轮没有执行存储删除。以下准备与开发状态属于已完成发行之前的历史记录。
...
**最新开发（2026-09-11，未发行）：** 失败业务步骤重试、结果分层、远端成果按需取回和更紧凑的对话已完成，保留此前简洁输入框改动。当前轮失败后可单独恢复步骤；新节点成果只同步清单，取回按钮或后续依赖才触发文件传输；仅引用结果的续聊不下载旧远端文件。380 项逻辑、13 项协议、双隔离服务及 18 组 UI 检查通过。详见 [WORKFLOW-RECOVERY-RESULTS](WORKFLOW-RECOVERY-RESULTS.md)。尚未提交、推送或发布；未操作安装版、真实项目/模型或官网。以下为此前开发与正式发布记录。

**后续开发（2026-09-10）：** “简洁输入框”已在本地完成：单行工具栏、按需附件/字数和独立会话设置弹窗，默认输入框高度由约 284px 减为 128px。373 项逻辑、13 组隔离 UI、双语/类型/构建及版本/测试分类检查通过。详见 [COMPOSER](COMPOSER.md)。未操作安装版、真实项目或模型；未发布新版。以下为上次 0.1.12 发布收尾记录。

更新：2026-09-10，会话已收尾。**0.1.12 已完成桌面正式发布、签名更新和官网同步。** 已发布桌面源码为 c039c7b8a3d43898d48ccedb469cbd79b68005ad，官网为 7daee76d1d560fcaafad1eb44557e373a01ec9c9。三组 Windows CI、隔离安装、公开包签名/哈希与正式域校验通过。当前新增工作仅为文档交接，未改变产品版本或安装包。

新会话先读 [NEXT-SESSION](NEXT-SESSION.md)：其中记录已完成功能、连续会话约定、可选后续功能、暂停的测试范围和代码入口。发布证据见 [VERIFICATION](VERIFICATION.md) 顶部；用户尚未选定下一项功能。不要把后面的旧候选记录作为当前版本，也不要恢复旧路线图中已被实现替代的限制。

## 历史交接记录

以下保留各轮当时的状态和证据；“最新候选”“未发布”“待 CI”等相对描述只适用于对应日期/版本。

> 2026-09-10：0.1.10 桌面（b778eaa1）与官网（87a54c85）已公开验证。用户要求去掉消息的“你 / 复制”栏、改为小型悬停复制图标并修复正文选择体验。本轮 0.1.11 已实现：普通/补充/工作流用户消息移除标题栏，消息与结果复制按悬停、键盘焦点、触屏和操作反馈显示；明确正文 user-select:text，选中底色由与气泡接近的 #edf2ff 改为 #bfd5ff。原始要求、换行、缩进和复制失败重试保留。生产 UI 14 项及逻辑 359 项检查通过；正式发布仍须云端安装与公开下载/签名核验。用户安装版、真实模型与项目文件继续不作为测试目标。
...
> 2026-09-10：0.1.8（0917e1c8）及官网已公开验证。本轮依用户要求实现 0.1.9：升级成功进入工作区后全部清理旧备份，包含旧版遗留和中断副本；失败启动及未来版本备份保留，文件占用每分钟重试。删除按钮悬停/焦点显示，触屏可见。独立安装 CI 增加真实工作区启动回执、完整/中断副本清理、重启与当前数据保留验收。沿用签名密钥；本地不操作用户安装版与现有备份、不调用真实模型。待本提交云端安装验收与公开发行验证。

> 2026-09-10：0.1.7（9a58d9ad）已公开发行，官网与签名验证通过。本轮依用户截图实现 0.1.8：目录别名按本机用户持久保存；默认短目录名，完整路径仍可查看和搜索；目录下会话缩进，删除整体“历史会话/总数”行；顶部在线区缩小并上移；中英文初始及切换语言后窗口标题均为 Rivloom。沿用 0.1.7 更新器和既有签名密钥，用户将验证 0.1.7 → 0.1.8；本地不操作用户安装版、不调用真实模型。待本提交云端安装验收与公开发行核验。

> 2026-09-10：0.1.6（4e2a2900）已公开发行，官网与签名清单验证通过。用户反馈从应用内升级时下载成功，但需点击更新弹窗 X 才继续安装；已明确为 Rivloom 内弹窗。本轮 0.1.7 在安装调用前释放更新及关于弹窗，原生持锁后台任务接管停机/备份/安装，提供分阶段文件进度并合并轮询。20 项原生测试（另 1 项 Explorer 手动测试忽略）、10 项生产 UI 桥接场景已通过，含无 X 交接、慢状态查询、过期结果拒绝、失败重试及中英文窄窗。待本提交 CI 与公开发行验证；不把浏览器桥接测试称为用户设备真实升级。保留原签名密钥与既有任务/数据保护。
...
**当时的本地包：0.1.5 / cd898d08**，已完成构建与 7736 个运行时文件解包一致性校验。[安装包](../.data/verification/desktop-updater-candidate-20260909/delivery/Rivloom_0.1.5_desktop_updater_20260909_cd898d08_x64-setup.exe)；[最终记录](../.data/verification/desktop-updater-candidate-20260909/delivery-result.json)。由用户自行安装，未发布。本提示在候选冻结后补写。
...
**会话结束补充：** 用户要求记录后续再改，本会话结束。真实协作测试中已定位的能力过期、文件折返与转交上下文问题已修复并通过针对性检查；剩余观察更偏模型指令遵循与生成质量，尚未完成另一模型的完整对照，不能全部归因于模型。完整业务验收仍待完成，但不直接等同于软件仍有确定缺陷。结果自动校验作为后续产品增强候选，不因本次模型失败成为必修项；具体依据与后续安排见 [NEXT-SESSION 验证范围](NEXT-SESSION.md#5-已验证和仍待验证)。本次只补文档，不继续开发或恢复暂停测试。
...
当前新会话开场白以 [NEXT-SESSION](NEXT-SESSION.md#给新会话的开场白) 为准。
