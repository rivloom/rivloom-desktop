...
6. **M3.3 远端任务邀请第一切片：自动化/Release 完成。** 归属 Brain 可向受信目标 Brain 加密发送邀请，对方本机所有者接受或拒绝，归属方可取消；双方保存全局任务 ID、幂等键、24 小时期限与投递状态。接受不绑定项目、角色、模型或 OpenCode，不会启动 AI。物理双机仍待用户复测。
...
- 大小：71,062,038 字节
- SHA-256：`3b627617174f84c8ef9c363b7b93d3455a418d6b3ca4c33b88d69c18ae967589`
...
- M3 Release WebView2：12 项通过；真实第二节点完成发现、同码、单方不授信、双方授信、加密任务邀请、人工接受、持久状态和撤销。自动化另覆盖 mDNS、LAN UDP、握手/密文攻击、任务幂等、重启和撤销。
...
- Win10 `192.168.5.18` 与 Win11 `192.168.5.20` 的新加密通道/任务邀请物理复测，以及异常进程结束/断网兜底专项复测；之后仍有项目授权、角色映射、执行租约、OpenCode 执行/审批/产物和两位真人验收。
- ChatGPT 登录。
...
若用户暂不提供 Key，让用户用当前安装包物理复测加密任务邀请、接受/拒绝/取消、重启保持和撤销取消；同时继续任务 8 的本机项目授权、角色映射、执行租约和事件序号，再逐步接执行/审批/产物。业务应用和引擎仍只监听 loopback；OpenCode 永远不直接暴露到局域网或公网。
...
> 请先阅读 `docs/HANDOFF.md`、`docs/PROGRESS.md`、`docs/adr/0001-self-discovering-brain-network.md` 和实施计划。先核对仓库状态，不重做 M0/M1/M2 或 M3 已完成切片，不使用过期 OpenCode Go，不修改旧项目。若我提供 DeepSeek Key，只指导我在客户端本机输入；否则先复测物理双机加密任务邀请，再继续任务 8 的项目授权、角色映射和执行租约，不做单主机邀请式协作。
