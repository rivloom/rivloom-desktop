> 2026-09-10：0.1.12 实现用户指定的四项功能：同一工作流会话 FIFO 追加消息、Markdown/代码高亮与复制、按用户和设备持久化草稿、粘贴/拖入/安全预览附件。完成且所有执行已结束后开始下一轮；停止或失败暂停消息队列，问题回答继续当前轮。归档轮次保留完整请求、回答、结果和文件，历史清理与更新阻塞覆盖队列。详情与验证范围见 CONVERSATION-CONTINUITY.md 和 VERIFICATION.md。正式发行仍须该提交云端 CI、隔离安装和公开下载/签名验证。未操作用户安装版、真实模型或项目文件。
