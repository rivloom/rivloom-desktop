...
- M3.1 Release WebView2：5 项通过；两个隔离实例经真实 mDNS、nonce 和 Ed25519 签名发现，附近节点保持未授权。
- 本地：TypeScript 检查与生产构建通过；9 项测试通过；Cargo fmt/clippy 通过；生产依赖离线 audit 为 0 个已知漏洞。
...
- M3 在两台物理设备上的发现验证、设备配对/撤销、加密业务通信、单 Brain 分布式任务、最小跨 Brain 委派，以及两位真人验收。同机双实例的身份和自发现已经完成。
- ChatGPT 登录。
