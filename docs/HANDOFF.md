...
6. **M3.3 远端任务邀请与执行准备：技术原型完成，产品流程待重做。** 归属 Brain 可向受信目标 Brain 加密发送邀请，对方本机所有者接受或拒绝，归属方可取消。当前包还要求接受方选择项目和模型，并创建 30 分钟准备租约；这套行为通过了自动化和 Release 验证，但用户已明确它不是目标体验。后续不得沿此流程继续扩展。
...
M3.3 又于 2026-09-01 修正：受信节点应能按本机预先配置的策略自动调用执行能力，也可以选择有限调用或每次确认；目标默认交互是自动调用。当前 30 分钟准备租约只是待迁移的原型，不应成为用户必须经历的步骤。下一实现入口见 [ADR-0002](adr/0002-configurable-node-invocation-policy.md)。
...
- 大小：71,024,519 字节
- SHA-256：`0f8a150c2e1f4f5cb20c096d5c99de8be66af61a5bf26dfe599a484944a0ed3c`
...
- M3 Release WebView2：16 项通过；真实第二节点完成发现/配对、加密任务邀请、本机项目/模型准备、远端隐私边界、项目拦截、准备撤销和设备撤销。自动化另覆盖记录迁移、准备自动过期、重启和攻击路径。
- 本地：TypeScript 检查与生产构建通过；14 项测试通过；Cargo fmt/clippy 通过；生产依赖离线 audit 为 0 个已知漏洞。
...
- Win10 `192.168.5.18` 与 Win11 `192.168.5.20` 的加密通道、任务邀请和本机准备租约物理复测，以及异常进程结束/断网兜底专项复测；之后仍有角色映射、可消费的执行租约、OpenCode 执行/审批/产物和两位真人验收。
- ChatGPT 登录。
...
若用户暂不提供 Key，下一步先按 ADR-0002 重做任务调用流程：建立可复用的本机能力配置，以及自动、有限、每次确认策略；不要继续升级当前 30 分钟准备租约。随后完成角色映射、有序事件和真实 OpenCode 执行，再逐步接审批、停止、差异和产物。业务应用和引擎仍只监听 loopback；OpenCode 永远不直接暴露到局域网或公网。
...
> 请先阅读 `docs/HANDOFF.md`、`docs/PROGRESS.md`、`docs/adr/0001-self-discovering-brain-network.md`、`docs/adr/0002-configurable-node-invocation-policy.md` 和实施计划。先核对仓库状态，不重做 M0/M1/M2 或 M3 已完成的底层协议，不使用过期 OpenCode Go，不修改旧项目。当前 30 分钟准备租约只是待迁移原型；下一步应实现受信节点的自动、有限、每次确认调用策略，默认目标是自动调用，再接角色映射、OpenCode 执行和事件回传。
