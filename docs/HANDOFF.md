...
- 下一会话直接讨论 M3.4。Brain/节点关系存在认识差异，讨论清楚前不得实现，也不得把历史方案视为已确认需求。
...
下一步直接讨论 M3.4，先明确 Brain、节点、人、任务、项目、模型额度和算力资源的关系，以及个人和企业场景需要的最小共同能力。定义确认前不写 M3.4 代码。M3.3 后续回归和防火墙体验保留在优化清单；业务应用和引擎仍只监听 loopback，OpenCode 永远不直接暴露到局域网或公网。
...
> 继续开发 `C:\project\rivloom-opencode`。先阅读 `docs/HANDOFF.md`、`docs/MILESTONES.md`、`docs/PROGRESS.md`、`docs/VERIFICATION.md` 和三份 M3 ADR，核对 Git 后继续，不重做已完成协议。M3.3 的 MVP 范围已验收完成：用户在 `192.168.5.20` 与 `192.168.5.33` 用真实 DeepSeek 完成单人双机文件生成、远程一次性批准、补充要求同会话续跑、本地核对和最终验收。防火墙安装体验、远程停止、另外两种审批、执行中撤销信任、异常断网和两真人角色回归都已记录为后续优化，不阻塞当前工作。这个会话不要先写代码，请直接和我讨论并重新定义 M3.4：Brain、节点、人、任务、项目、模型额度和算力资源分别是什么，个人与企业如何共用同一最小模型，一个或多个 Brain 如何形成并协同做事。不要沿用未经确认的“每节点一个 Brain / Brain A 委派 Brain B”假设。讨论达成共识后先更新 ADR、里程碑、最小闭环和验收标准，再开始实现。不修改或 fork OpenCode，不要求 Git，不用文件快照或哈希。
