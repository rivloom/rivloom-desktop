...
- 大小：71,039,431 字节
- SHA-256：`eae0ad3a809fafeadcd69e2e6c4c8272ac136b9df241467dc4d28f5bafd88c87`
...
- M3.1 Release WebView2：5 项通过；节点经局域网发现、nonce 和 Ed25519 签名验证，附近节点保持未授权。单元测试分别强制验证 mDNS 与 LAN UDP 回退。
- 本地：TypeScript 检查与生产构建通过；10 项测试通过；Cargo fmt/clippy 通过；生产依赖离线 audit 为 0 个已知漏洞。
...
- `desktop-node-network.json` 与 `desktop-node-network.png`

## 明确未完成

- **DeepSeek 真实调用尚未验证。** 用户尚未提供有效 Key。测试字符串接口检查不能写成认证成功、额度可用或真实回复成功。
- 新生命周期版本在 Win10 `192.168.5.18` 与 Win11 `192.168.5.20` 上的正常关闭、异常断线、移除、恢复及身份稳定复测；之后仍有设备配对/撤销、加密业务通信、单 Brain 分布式任务、最小跨 Brain 委派，以及两位真人验收。
- ChatGPT 登录。
...
若用户暂不提供 Key，先用第二台 Windows 设备验证 M3.1 的真实局域网和防火墙行为，再进入 M3.2 双方确认配对、持久信任与撤销。业务应用和引擎仍只监听 loopback；目前局域网只开放返回签名公开身份的 Rivloom 节点端点，OpenCode 永远不直接暴露到局域网或公网。
