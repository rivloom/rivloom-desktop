...
6. **M3.3 远端任务邀请与执行准备：自动化/Release 完成。** 归属 Brain 可向受信目标 Brain 加密发送邀请，对方本机所有者接受或拒绝，归属方可取消。接受方随后只在自己的设备选择已授权项目和可用模型，用 30 分钟准备租约保留项目；发起方只收到状态和期限。准备租约会拦截该项目的普通任务启动，可撤销、自动过期并在重启/断线后恢复；不会创建 OpenCode 会话。物理双机仍待用户复测。
...
若用户暂不提供 Key，让用户用当前安装包物理复测任务邀请、本机项目/模型准备、准备撤销、重启保持与设备撤销；同时继续任务 8 的角色映射、一次性执行租约和有序事件，再逐步接 OpenCode 执行/审批/产物。业务应用和引擎仍只监听 loopback；OpenCode 永远不直接暴露到局域网或公网。
...
> 请先阅读 `docs/HANDOFF.md`、`docs/PROGRESS.md`、`docs/adr/0001-self-discovering-brain-network.md` 和实施计划。先核对仓库状态，不重做 M0/M1/M2 或 M3 已完成切片，不使用过期 OpenCode Go，不修改旧项目。若我提供 DeepSeek Key，只指导我在客户端本机输入；否则先复测物理双机任务邀请与本机准备租约，再继续任务 8 的角色映射、一次性执行租约和事件回传，不做单主机邀请式协作。
