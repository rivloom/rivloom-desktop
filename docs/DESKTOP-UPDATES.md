...
**2026-09-11：** 源码递增至 0.1.13，沿用原更新公钥与签名发布链路；按需远端成果和远端安全步骤重试需要双方支持新能力，旧节点保留原有回传规则。发布结果见维护者本地验证记录。

更新：2026-09-10。源码版本 `0.1.12`，Windows x64 / Rivloom / `com.rivloom.desktop`。0.1.12 已公开发行并通过完整 CI、隔离安装、匿名下载和签名核验，沿用原更新密钥；发行证据见维护者本地验证记录。本版增加连续会话、Markdown、草稿和附件预览，自动接续的待执行消息会阻止更新交接。
...
- 安装包：Rivloom 0.1.5 更新功能候选，仅维护者本地保留，非公开下载。
...
- 本段及维护者本地交接记录指向最终包的文字在冻结后补写，仅文档变更；不改变冻结快照与安装包，不把当前整棵工作区重新声明为该 digest。
