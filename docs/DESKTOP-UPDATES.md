...
更新：2026-09-10。源码版本 `0.1.7`，Windows x64 / Rivloom / `com.rivloom.desktop`。0.1.5、0.1.6 已公开发行，原签名更新通道已启用并验证。0.1.7 修复安装阶段依赖更新弹窗关闭的问题，发行须通过该提交完整 CI 和公开清单核验。下方 0.1.5 本地候选记录保留为历史资料。
...
安装前保存 `pending-install.json`；下次版本已达到目标则清除，仍是原版本则显示上次安装未完成。SQLite 读入前拒绝高于当前支持的 `user_version=4`，不改写未来格式。本次没有改变数据 schema，也未实现任意版本间的有序迁移、自动回滚或用户可操作的恢复工具。
