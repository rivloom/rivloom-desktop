...
**2026-09-11：** 源码递增至 0.1.13，沿用原更新公钥与签名发布链路；按需远端成果和远端安全步骤重试需要双方支持新能力，旧节点保留原有回传规则。发布结果见 [VERIFICATION](VERIFICATION.md) 顶部。

更新：2026-09-10。源码版本 `0.1.12`，Windows x64 / Rivloom / `com.rivloom.desktop`。0.1.12 已公开发行并通过完整 CI、隔离安装、匿名下载和签名核验，沿用原更新密钥；发行证据见 [VERIFICATION](VERIFICATION.md) 顶部。本版增加连续会话、Markdown、草稿和附件预览，自动接续的待执行消息会阻止更新交接。
