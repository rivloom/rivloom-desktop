...
- 受信节点自动用 Ed25519 签名临时 X25519 握手，并以 HKDF 与 AES-256-GCM 建立 10 分钟会话；消息绑定双方身份、方向、严格序号和时间窗。桌面会区分“正在建立加密通道”和“加密通道就绪”。当前同步 Brain 目录，并支持任务邀请、人工接受/拒绝/取消与断线重试；接受后不绑定项目、模型或启动 AI。
...
对内测试安装包：`src-tauri/target/release/bundle/nsis/Rivloom_0.1.0_x64-setup.exe`。安装后从开始菜单启动 Rivloom。当前产物为 71,062,038 字节，SHA-256：`3b627617174f84c8ef9c363b7b93d3455a418d6b3ca4c33b88d69c18ae967589`。
...
真实模型回归命令会打开桌面窗口，在独立测试目录自动建立本机操作者、调用真实模型、验证审批/验收/重启，并模拟异常退出。节点网络命令也打开实际 Release WebView2，同时启动第二个隔离的真实 Rivloom 节点，通过局域网发现、双端配对、认证加密通道、任务邀请、人工接受和撤销完成验证。单元测试分别强制关闭 LAN UDP 和 mDNS，并覆盖握手/密文攻击、任务幂等、持久化、重启和撤销。不会操作个人工作区；同机双实例不等于两个真人或两台物理设备已验证。
