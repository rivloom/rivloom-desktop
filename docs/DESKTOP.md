...
- M3.1 的发现、正常退出和 M3.2 的配对闭环已由用户在 Win10 `192.168.5.18` 与 Win11 `192.168.5.20` 确认。认证加密通道、Brain 目录、策略化真实执行以及远程批准/回答/停止/补充、官方差异和验收已通过协议与实际 Release WebView2 的相应检查；物理双机执行和两真人角色验收仍待用户复测。异常结束进程或断网的 15/30 秒兜底仍待专项复测。专用网络首次发现可能出现 Windows 防火墙提示。
