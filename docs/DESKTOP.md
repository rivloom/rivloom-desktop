...
- 附近节点约每 10 秒刷新签名心跳；连续 30 秒无成功心跳显示离线，120 秒后移除，重新出现时自动恢复在线。顶部和侧栏只统计在线节点。
...
对内测试安装包：`src-tauri/target/release/bundle/nsis/Rivloom_0.1.0_x64-setup.exe`。安装后从开始菜单启动 Rivloom。当前产物为 71,010,576 字节，SHA-256：`3fb676844d87998cc49174fbe50a49d215b24c05fa40b3607e5224cb1b2ecc5b`。
...
- M3.1 已验证同机双实例的 mDNS、LAN UDP 回退和身份签名。原安装包在 Win10 `192.168.5.18` 与 Win11 `192.168.5.20` 实测出现单向 mDNS；LAN UDP 回退包随后已在相同两台机器确认双向发现。现场继续发现原 120 秒保活期间仍显示在线，现已拆分为 30 秒离线、120 秒移除，等待原设备复测关闭与恢复。配对/撤销、加密业务通信和任务委派仍待后续里程碑。专用网络首次发现可能出现 Windows 防火墙提示。
