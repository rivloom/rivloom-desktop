...
对内测试安装包：`src-tauri/target/release/bundle/nsis/Rivloom_0.1.0_x64-setup.exe`。安装后从开始菜单启动 Rivloom。当前产物为 71,044,500 字节，SHA-256：`d186d258a434a547692a52250e7e6170e55649a2f5ad4ef77f7051e0eda21dcf`。
...
- M3.1 已验证同机双实例的 mDNS、LAN UDP 回退和身份签名。原安装包在 Win10 `192.168.5.18` 与 Win11 `192.168.5.20` 实测出现单向 mDNS；LAN UDP 回退包随后已在相同两台机器确认双向发现。现场继续发现原 120 秒保活期间仍显示在线；现已增加正常关闭的签名离线通知，并将异常断线兜底改为约 15 秒离线、约 30 秒移除，等待原设备复测正常关闭、异常断线与恢复。M3.2 配对第一切片已通过同机真实双实例和 Release WebView2，物理双机配对/撤销仍待用户验证；认证加密业务通信和任务委派尚未开放。专用网络首次发现可能出现 Windows 防火墙提示。
