...
- 受信节点自动用 Ed25519 签名临时 X25519 握手，并以 HKDF 与 AES-256-GCM 建立 10 分钟会话；消息绑定双方身份、方向、严格序号和时间窗。桌面会区分“正在建立加密通道”和“加密通道就绪”。当前只同步 Brain 目录和粗粒度能力，任务委派尚未开放。
...
对内测试安装包：`src-tauri/target/release/bundle/nsis/Rivloom_0.1.0_x64-setup.exe`。安装后从开始菜单启动 Rivloom。当前产物为 71,009,359 字节，SHA-256：`a198a72c0403a5eaff69f8afe6c3b91446b7cac6611b6599693f5973a89126d1`。
...
真实模型回归命令会打开桌面窗口，在独立测试目录自动建立本机操作者、调用真实模型、验证审批/验收/重启，并模拟异常退出。节点网络命令也打开实际 Release WebView2，同时启动第二个隔离的真实 Rivloom 节点，通过局域网发现、双端配对、认证加密通道和 Brain 目录往返完成验证。单元测试分别强制关闭 LAN UDP 和 mDNS，并覆盖伪造握手、握手/消息重放、密文篡改、过期和撤销。不会操作个人工作区；同机双实例不等于两个真人或两台物理设备已验证。
