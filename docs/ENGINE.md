...
| 结果             | `session.messages`，业务层保存可显示快照                                   |
| 引擎差异         | `session.diff`，本机返回空数组，明确降级                                   |
| 产品产物         | 只读 Git diff + 非忽略的新文件 + 验收指纹                                  |

没有调用 OpenCode 内部模块，没有自写 Agent 循环。业务层的定时同步只查询状态和保存 UI 快照，不生成提示循环或执行工具。

## 真实验证发现

1. 现有 OpenCode Go 账号返回余额不足；没有充值。使用官方免费提供方 `opencode/mimo-v2.5-free` 完成同一个真实测试。
2. `session.diff` 多次返回空数组，但真实文件修改和 Git diff 正常。MVP 缩减为 Git 可见产物审阅，不进入引擎内部修复。
