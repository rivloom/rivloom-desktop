...
`rivloom_history` 支持状态读取、按轮/步骤/关键词检索及固定版本分页回读；搜索每页最多 10 条预览，正文每页最多 4,000 个 UTF-16 单位。默认交接携带当前需求、有效条目、当前步骤与直接依赖的进度/文件来源；缩短的检查点明确标记，保留取回引用。完整原始需求与有效条目放不下时拒绝准备，不静默裁掉关键约束。
