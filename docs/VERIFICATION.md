...
修复依赖引导按 Node 查找方式向上定位锁定的 plugin 依赖，继续限制复制文件在所选依赖树内；新增真实隔离副本导入 plugin/tool 与 Zod 的回归。三节点夹具等待双向签名 Brain 成员信息完整传播后再验证知识库，未放宽私有不可见和共享数量断言。进一步定位到异步协作查询期间，反向 Brain 目录消息可能推进接收序列，导致合法回执被拒绝；现在在异步处理前绑定已验真的请求摘要与序列，完成后沿用原域隔离密钥和回执协议加密。确定性回归先复现失败，再验证修复；重放、错配、篡改、过期与大小限制继续生效。

| 修复后检查 | 结果 |
| --- | --- |
| 完整逻辑 | 416/416；`test-results/ci/tests-logic-1789459935888-a98404e9.json` |
| 协议 | 13/13；`test-results/ci/tests-protocol-1789460013258-f58434a0.json` |
| CI 自检与生产构建 | 69/69；类型、双语、构建通过，原有大 chunk 提示保留 |
| CI 原入口知识库服务 | 6 组三节点检查、8 次本机模拟模型请求通过；`test-results/ci/service-knowledge-1789459972016-83e25651.json` |
| 诊断压力复测 | 独立临时源码副本连续 24 次远端读取，再跑完整 6 组服务与 8 次模拟模型请求，全部通过；诊断日志未进入产品代码 |
| CI 原入口模型服务 | 12 组通过，覆盖本机 Chat Completions / Responses；未使用真实账号或额度 |

原始失败、诊断及通过日志保存在被忽略的 `.data/verification/release-0.1.15-20260915/`。下一步推送修复后重新运行该提交的全部发布门禁；不能使用首个提交的部分通过结果替代修复后的完整 CI、安装和公开验签。

## 2026-09-15 · 0.1.15 发布准备

用户已授权版本加一、提交与升级。本版包含共享 Skills、渐进式 Wiki 记忆及此前完成的许可/发布来源保护修复。package、npm 锁文件、Cargo、Cargo 锁文件和 Tauri 统一为 0.1.15；新增对应更新说明。功能回归沿用下方 414 项逻辑、68 项 CI 自检、13 项协议、3 项引擎和 6 组三节点服务通过证据；本版云端构建、安装、发布、签名与 R2 收尾尚待实际运行，不以历史结果代替。

合成预览补充了远端素材 Node 分享的“素材交付检查” Skill 和支持文件，以便查看 Brain 中继与渐进读取。补充后类型检查通过；本次重开预览的 Computer Use 因无法可靠确定浏览器地址而停止，未记为新的完整界面验收；此前 09-14 的界面验证结果仍保留。未调用真实模型。

0.1.15 提交前重新通过版本一致性、完整测试分类、68/68 CI 自检、1318 UI/native + 602 系统翻译键、类型检查、生产构建和 diff 检查。保留已有大 chunk 提示。远端 main 核对为 95a545d759956ba7fd43529e85c8ee2bc1dab20f，无 ci-v0.1.15 标签；仓库仍为私有。

## 2026-09-15 · 用户确认撤销分享，当前文档引用清理完成

用户明确确认“我确定已经删了”，据此记录为账号持有人已确认在 ChatGPT 内撤销该分享。此前匿名访问超时及浏览器连接失败仍是未独立验证状态，不把用户确认改写成匿名页面验证通过。

已移除 MILESTONES、PROGRESS、RELEASING 和 ADR 0006 中的四处链接，保留讨论依据与既有发行事实；开源准备、CI 和交接文档同步更新。按 Git 跟踪及未忽略文件范围扫描，当前文件已无 ChatGPT 分享 URL，`git diff --check` 通过；Git 历史及被忽略的本机审计记录保留。未修改 Git 历史、标签、工作流、安装包或远端配置，HEAD 保持 `95a545d`；保留 main 自动发布，本轮未提交、推送或公开仓库。

此轮仅修改文档，未重复运行产品测试。下方记录是当时检查结果，分享撤销与引用清理状态以本段为准；不再要求用户重复确认或选择迁移/历史重写方案。

## 2026-09-15 · 夜间开源补充审计（本地收尾，分享失效未确认）

按用户“十分钟后核验并继续收尾”的要求执行。用户已转为先关闭分享、尽量保留现有仓库和 CI/CD；本轮未迁移仓库、改写历史、提交/推送或发布。

| 检查 | 实际结果 |
| --- | --- |
| 分享链接 | 约定时间后匿名 curl 两次超时，Node 请求连接超时，网页读取未取得内容，浏览器服务不可用。**无法确认取消成功，也未证实仍可访问。** 当前 4 份文档引用未删除，37 个历史提交与 15 个标签未改写。 |
| 云端下载完整性 | 全部 **399 个 artifacts** 的大小与 SHA-256 对齐 GitHub 元数据；102 份日志与 384 份小型 artifacts 的 ZIP CRC 通过。日志只记录本机摘要，GitHub 未提供预期摘要。 |
| Release 校验文件 | **15/15** 取得并匹配附件摘要和对应安装器摘要，补齐之前缺失的 0.1.11。Gitleaks 重扫无命中。 |
| 大型 ZIP / 安装器 | **15/15** 候选 ZIP 复用已验证安装器字节并取得远端其余原始字节，重建后的整包 SHA-256 与 GitHub 完全一致。120 个条目 CRC 通过，内含安装器均匹配对应 Release；15 个安装器均通过大小/哈希核验，其中 14 个复用缓存，1 个从官网匿名补取。 |
| 静态内容扫描 | 官方校验过的 7-Zip 26.03 解出 **121,073 个文件 / 5.52 GB**；加上 105 份候选报告、当前 60 个与历史 99 个二进制对象，去重 **9,730 个文件**。Gitleaks 扫描文本与 ASCII/UTF-16 字符串 **211.10 MB**。16 项 Gitleaks、9 项补充规则命中均已逐项复核为上游样例、占位符、公开客户端 ID 或代码误报，无新增真实凭据发现。 |

审计中没有执行安装器或内含程序。NSIS 清单所列 StartMenu.dll 大小与实际解包大小的差异已记录；不把它当作每文件大小完全一致。静态提取不能保证识别所有个人信息或加密/压缩区段。详细证据与方法见 [开源准备](OPEN-SOURCE-READINESS.md) 和本机 `.data/open-source-20260914/installer-triage.json`、`all-downloads-verified.json`、`share-verification.json`。

本次新增修改仅为审计及交接文档，产品代码验证沿用下方已通过的 414 项逻辑、68 项 CI 自检及独立构建；未把已有发布或旧安装验收当作本轮新代码的发行结果。收尾重新扫描当前源码，仍只有已复核的公开测试验签公钥命中；`git diff --check` 通过，HEAD 保持 `95a545d`。一次性提醒已暂停，不继续安排重复检查。仓库继续私有，main 自动发布保留，真实账号/模型额度、用户安装数据、R2 清理和官网部署均未操作。

## 2026-09-14 · 开源准备修复（未提交、未推送、未公开）

用户确认 Apache-2.0 / Copyright 2026 Rivloom contributors；官网继续私有、两 runtime fork 只查配置；公开时启用 GitHub 私密漏洞报告；保留 main 自动发布。分享链接历史处理尚未选择，当前不能公开。

| 检查 | 实际结果 |
| --- | --- |
| 发布与许可回归 | 针对性 23/23；包含官方仓库/来源、PR 权限、artifact 来源、LICENSE/NOTICE 缺失及重算哈希后的篡改拒绝。 |
| CI 自检 | 68/68，零跳过；新增 3 项工作流策略测试。 |
| 逻辑回归 | 414/414，零跳过；`test-results/ci/tests-logic-1789398229538-7ce78c43.json`。 |
| 构建 | 类型、1318 UI/native + 602 系统翻译键、生产构建、版本与测试分类通过；原有大 chunk 提示保留。 |
| 独立源码副本 | 本地 Git 克隆覆盖待审源码，全新 npm ci --offline、构建及 68 项 CI 自检通过；没有复制安装版数据、既有 node_modules 或 runtime。msgpackr-extract 安装脚本未批准提示已记录，不作为完整原生包验收。 |
| runtime | 实际准备与门禁通过：官方 Node/OpenCode 哈希、221 个 npm runtime 包、316 个 Rust 依赖、945 份许可文件，含自有 LICENSE/NOTICE。没有编译或运行新安装器。 |
| 内容审计 | 官方 Gitleaks 8.30.1 + 补充规则，63 个历史提交；Gitleaks 仅命中公开测试验签公钥，人工复核为误报。已取得日志/报告与 Release 校验文件未命中。完整数量、未取得附件和未扫二进制边界见 [开源准备](OPEN-SOURCE-READINESS.md)。 |

原始审计、下载和测试记录留在被忽略的 `.data/open-source-20260914/`。同一 ChatGPT 分享链接涉及 4 个文件、37 个历史提交及 15 个现有发布标签；用户明确不允许公开，历史清理先讨论，未修改引用、重写历史或远端。没有真实账号/模型额度调用、R2 清理、官网部署或仓库可见性变更。下方 Skills/Wiki 和 0.1.14 发布证据继续作为各自历史状态保留。

## 2026-09-14 · 共享 Skills 与渐进式记忆（工作区未发布）

基于已发布 0.1.14 完成第一版，尚未提交、推送、打包、安装或发布。用户确认“记忆默认本机，手动分享给 Brain”。使用方式及边界见 [KNOWLEDGE-LIBRARY](KNOWLEDGE-LIBRARY.md)。后台整理当前重建分类索引和标记完全重复正文；语义分类由用户或任务中的 Agent 完成，未新增独立 AI 后台整理。

| 检查 | 实际结果 |
| --- | --- |
| 逻辑回归 | **413/413，零跳过**；`test-results/ci/tests-logic-1789383766041-9daa3301.json`。新增 10 项覆盖默认私有、项目范围、历史/版本冲突、批量分享原子性、链接拒绝、中继、撤权、离线、篡改和任务授权。 |
| 三节点服务 | **6 组通过，8 次请求全部为本机模拟模型**；`.data/knowledge-service/1789383197796/report.json`。A 与 B 无直接配对；Brain 目录、最新版读取、官方引擎实际执行 Skill 脚本、共享 Wiki/私有记忆、两层说明、审批拒绝、撤销分享、关闭发现后重启持久化均验证。 |
| 加密协议 | **13/13，零跳过**；`test-results/ci/tests-protocol-1789383326194-0eccfcdb.json`。 |
| 引擎隔离 | **3/3，零跳过**；`test-results/ci/tests-engine-1789383292234-2ee282e9.json`，含 OAuth 临时引擎与主凭据隔离。 |
| 页面验收 | computer-use 操作生产构建、真实知识库 API 和合成节点，覆盖分享状态、Brain 内容、共享 Wiki、新建、编辑历史、分类分享、主说明、中英文、390px 和非法登记错误；浏览器 error 日志为空。`.data/verification/knowledge-20260914/ui-report.json`。 |
| 构建门禁 | TypeScript、**1318 UI/native + 602 系统翻译键**、生产构建、版本、覆盖及 **65 项 CI 自检**通过。原有大 chunk 提示保留。服务矩阵增加 knowledge，尚未运行本份代码的云端 CI。 |
| 依赖 | 官方插件 SDK 与引擎均锁定 **1.18.25**，首次启动从本地运行时准备依赖，避免本地插件等待联网安装；二进制未修改。重新生成 **253 个依赖版本、259 份许可证文件**；生成类型包另保留发布许可声明与对应 Kubernetes 原始许可。 |

最初沙箱内 DPAPI/网络受限，隔离重跑后通过。首次插件等待依赖安装的问题已修复；早期失败报告保留在 `.data/knowledge-service/`，不计为通过。最后浏览器自动审批曾超时，一次允许的重试成功，未绕过策略。

这些结果不代替双物理机、安装包或真实模型效果验收。没有真实账号登录、真实额度调用、用户安装版数据操作或 R2/官网发布。

## 2026-09-14 · 0.1.14 发布完成
...
- [发布流水线](https://github.com/rivloom/rivloom-desktop/actions/runs/34817618666)；源码 `0f8d8b5b68f8df6357a8deaec8ead7be29d39465`。基础 CI 34817436920、官方引擎 34817436717、发现/传输 34817436747 均成功；403 项逻辑、13 项协议、3 项官方引擎与 9 组服务检查通过，模型服务报告 12 组、工作流服务 21 组。发布前版本、覆盖、双语、构建和 65 项 CI 自检通过。
- 公开安装包 **82,446,061 字节**，SHA-256 `27b33d834d811d84144331c821e72aa9ee3ea4a0063c6130ea5b71c0cd8473da`。匿名完整下载、原更新公钥、签名清单与安装器绑定、版本清单/latest 一致及公开 SHA256SUMS 均另行验证；未运行用户安装版或在用户设备执行安装器。
- 官网中英文模型接入指南、搜索说明、更新日志和隐私同步；41 项测试、18 页构建、12 组说明页与 4 组下载页桌面/手机检查通过。实际官网部署与 23 项线上检查通过，下载页引用与签名更新一致。官网源码 `269d9ead1e3e988f08a294f748ca9dd1514dd5b4`。
- R2 只读 inventory [34819797965](https://github.com/rivloom/rivloom-desktop/actions/runs/34819797965) 成功。最近三版 0.1.14、0.1.13、0.1.12，与近 30 天/当前引用取并集，逐项结果见 [R2 检查记录](releases/0.1.14-r2-retention.md)。检查完成，删除 0 个；只读清单入口没有删除功能，不是自动清理。
- 新功能本地 UI、取消/超时、凭据隔离和自定义双协议真实本机模拟服务证据沿用下方开发记录。未登录真实厂商账号，未消耗真实模型额度，不把入口可用等同所有厂商实测成功。

本机完整证据：`.data/verification/release-0.1.14-20260914/`；官网证据：`C:/project/rivloom-website/.data/verification/model-release-20260914/`。GitHub 大型候选压缩包整包下载两次超时，后通过精确 HTTP 字节范围取得原始 7 份 JSON 并核对 ZIP 条目 CRC；与匿名下载的完整安装包一起，再次通过现有 `prepareRelease` 的严格源码、runtime、CI、安装与哈希门禁，见 `candidate-verification.json`。这不代表整份 ZIP 的 SHA-256 已在本地重算。
...
## 2026-09-14：模型接入开发阶段验证（历史）

在会话搜索改动之后完成模型接入。当时正式版仍为 **0.1.13 / 14ee393**，未提交、推送、发布或安装；此前 R2 文档与搜索改动保留。

| 检查 | 结果与证据 |
| --- | --- |
| 逻辑回归 | 403/403，零跳过；`test-results/ci/tests-logic-1789315919748-4c3ba11c.json`；新 Provider/OAuth 单测 10 项 |
| 官方引擎隔离 | 3/3，零跳过；`test-results/ci/tests-engine-1789315959780-16c8f1f6.json`；含 OAuth 临时引擎与主凭据隔离、进程退出及目录清理 |
| 模型服务 | 12 组；`.data/verification/model-access-20260913/service-report.json`；真实应用及官方引擎，3 次请求全部发往本机模拟接口，覆盖两种协议、认证、免 Key、精确 ID、编辑、重启、移除、默认回退及成员权限 |
| 模型 UI | 10 组；同目录 `ui-report.json`；OAuth/条件字段/授权码/取消、多模型编辑、默认模型、会话选择、Secret 清空、中文英文与 390px；无浏览器运行时错误，无自动模型测试 |
| 搜索回归 | 14 组；`.data/verification/conversation-search-20260913/ui-report.json`；保留原验收断言，修复清空搜索时 deferred render 短暂保留旧匹配的问题 |
| 构建与门禁 | TypeScript、1248 UI/native 与 602 系统翻译键、生产构建、测试分类、版本一致性及 diff 检查通过；原有大 chunk 提示保留 |
| 交付 | 同目录 `preview-dist` 和 `打开模型接入验收.cmd`；纯内存合成预览，不连接厂商、不保留 Key、不执行模型 |

OAuth 生命周期的厂商响应使用可控 Driver 验证；没有用真实账号完成厂商 OAuth，也没有把“目录有 OAuth 入口”当成全部厂商已登录通过。官方引擎的暂存隔离与清理由独立真实引擎测试覆盖。Windows 沙箱内 taskkill 曾返回“拒绝访问”，该次测试如实失败；在自动审批允许的沙箱外，仅重跑隔离引擎测试后确认 3/3 通过，没有降低退出确认要求。未修改安装版、真实项目、真实额度或恢复暂停的物理机/原生性能测试。

缓存研究已核对 OpenAI、Claude API、Claude Code、DeepSeek 官方文档及本仓库的系统/用户消息组织方式；未声称 Rivloom 有已测得的高缓存命中率，也未实现或承诺缓存优化。[模型使用说明](MODEL-ACCESS.md) · [缓存研究](PROMPT-CACHING.md)。

## 2026-09-13：会话内容搜索，待用户验收

本轮按用户授权完成会话内容搜索与命中定位。正式发行基线为已发布的 **0.1.13 / 14ee393**；下面旧发布准备段落为历史记录。本轮代码未提交、推送、发布或安装，已有 R2 文档改动全部保留。

| 检查 | 结果与证据 |
| --- | --- |
| 全部逻辑回归 | 393/393，零跳过；`test-results/ci/tests-logic-1789313360313-cfb21474.json` |
| 搜索与步骤状态定向 | 20/20；`.data/verification/conversation-search-20260913/targeted.log`；其中新增 12 项搜索检查及 1 项历史执行标签回归 |
| 生产界面搜索 | 14 组通过；同目录 `ui-report.json`；历史轮次/旧尝试/长消息/待执行消息定位、上下切换、清空/退出/Enter/组合输入、筛选、删除后消失/恢复、草稿、代码原文复制、中英文和 390px |
| 输入框回归 | 13 组通过；同目录 `composer-regression/ui-result.json`；覆盖附件、草稿、发送与续聊、设置、焦点、中英文及 1280/900/420/360px |
| 构建与静态检查 | TypeScript、1200 UI/native 和 586 系统翻译键、生产构建通过；0.1.13 版本一致性、测试分类与 `git diff --check` 通过；原有大 chunk 提示保留 |
| 合成搜索基准 | 一万条会话、五万片段、50,738,905 字符；索引 19.9ms、单次查询 2.8ms，找到末尾唯一匹配；同目录 `performance.json` |
| 交付预览 | 同目录 `preview-dist` 冻结构建与 `打开搜索验收.cmd`；源文件和构建摘要记录在 `source-hashes.json` |

截图已检查，搜索 UI 和输入框回归均无浏览器运行时错误。搜索过程未发出任务执行、步骤重试、队列控制或远端取回请求。新索引只基于当前鉴权且经过历史过滤的 bootstrap，在内存中随数据更新；不扫描用户文件，不新建持久化搜索数据库。历史实际删除/恢复边界由 SQLite 单测覆盖，UI 使用合成数据快照模拟移除与恢复。

界面测试修正了夹具中错误的输入框选择器、对滚动到末尾的过强顶端对齐断言、浏览器默认语言假设和移动侧栏可见性判断，没有放宽“关键词实际进入可视范围”的断言。复核修复了产品中旧记录缺少用户消息时无需求定位入口，以及历史执行状态误用当前步骤失败标签的问题。

全部为本地合成记录与隔离浏览器验证；没有运行模型、修改安装版/真实项目或进行双物理机/原生 WebView/安装更新验收。算法基准不代表全量历史渲染或长期原生性能。使用说明与人工验收清单见 [CONVERSATION-SEARCH](CONVERSATION-SEARCH.md)。临时测试浏览器和服务均已关闭，预览通过启动入口按需运行。

## 2026-09-11 后续开发：步骤重试、远端成果与紧凑结果，尚未发布
...
本地实现当前轮失败业务步骤重试、结果/文件/过程分层、远端成果按需取回及更紧凑的对话。保留此前记录的简洁输入框改动，版本仍为 0.1.12，未提交、推送、打包或发布。说明见 [WORKFLOW-RECOVERY-RESULTS](WORKFLOW-RECOVERY-RESULTS.md)。

| 验证 | 结果与证据 |
| --- | --- |
| 逻辑回归 | 380/380，零跳过；`test-results/ci/tests-logic-1789056727641-8880a124.json` |
| 最终定向回归 | 69/69，零跳过；`.data/verification/retry-results-targeted-final.log`；覆盖最后收紧的单文件重试范围 |
| 加密与兼容协议 | 13/13，零跳过；`test-results/ci/tests-protocol-1789056291855-76eca6da.json`；含异步认证请求与旧解码器兼容 |
| 官方引擎双隔离服务 | `.data/workflow-service/1789056184591/result.json`；本机/远端失败步骤只重试一次、保留前置、远端双文件零自动传输、所选文件取回、依赖中转，以及原审批/并行/转交/连续会话/重启测试通过 |
| 取回重启与离线 | 同目录 `retrieval-restart-result.json`；原一份已到本机、一份远端的状态跨重启保留；源端离线不能误报完成，重连后可显式取回 |
| 后续会话零自动下载 | 同目录 `metadata-followup-result.json`；完整后续规划和执行业务只使用会话记录，两份历史成果仍为 remote，源端没有传输队列 |
| 结果 UI | `.data/verification/retry-results-20260910/ui/ui-result.json`，5 组通过：Markdown/复制、按文件取回与失败重试、多轮密度、390px 英文、部分成果与步骤重试；零浏览器错误 |
| 输入框回归 | 同目录 `composer-regression/ui-result.json`，13 组通过，保留草稿/附件/发送/设置/焦点及中英文 1280/900/420/360px 行为 |
| 构建与审计 | TypeScript、生产构建、1184 UI/native 与 586 系统翻译键通过；0.1.12 版本一致性与测试分类审计通过；已有大 chunk 提示保留 |

新增逻辑测试覆盖重试回执跨重启、防重复、旧版本/轮次竞争、未确认执行拒绝、独立失败分支、材料未齐不派发、传输策略不可降级、旧路由自动回传、校验失败缓存的严格来源重试，以及单文件重试不重新启动别的失败文件。

初次默认沙箱运行协议/服务测试遇到 Windows DPAPI 无法保护临时节点私钥，节点网络未启动；随后在可使用 DPAPI 的测试进程中重跑通过，没有改弱产品身份保护。完整日志保留在 `.data/verification/retry-results-protocol*.log` 及两次服务目录。

全部后端验收使用独立 `.data` 目录和回环合成模型；额外重启检查不发送模型请求，后续会话检查仍仅使用合成模型。截图逐张核对，修正了窄窗来源路径被取回按钮挤压的问题。临时服务、引擎与浏览器已停止。未访问用户安装版/真实项目/真实模型，未进行原生 WebView、安装更新或双物理机验收。旧节点仍按旧规则自动回传；按需成果和远端安全重试需两端支持新能力。

## 2026-09-10 后续开发：简洁输入框，尚未发布

生产构建的空输入框在 1280px 视口下从 284.19px 缩至 128px。Edge 合成 API 夹具 13 组检查通过：常用操作单行排列；设置弹窗、Esc 与清除目标后的焦点恢复；Node 首选/锁定；输入法和换行/发送；上限提醒；紧凑附件、预览关闭不发送、粘贴/拖入事件；上传失败/重试/移除；草稿与附件刷新恢复、失败发送保留；同工作流追加；中英文 1280/900/420/360px；5 个长文件名滚动和第 6 个文件的限额提示。截图已核对，零浏览器运行时错误。

本次修复了设置弹窗移除后焦点丢失，以及窄窗目标设置一行内挤压设备名称的问题。上传失败场景初次夹具误将故障注入未完成的移除请求，已将故障限定到上传路径后通过，没有放宽断言。

逻辑 373/373、零跳过，报告 `test-results/ci/tests-logic-1789053427334-3218e7a7.json`；生产构建、TypeScript、1175 UI/native 和 585 系统翻译键、0.1.12 版本一致性及测试分类审计通过。证据 `.data/verification/composer-20260910/`，说明见 [COMPOSER](COMPOSER.md)。既有构建的大 chunk 提示仍存在。

全部使用独立浏览器上下文与本机回环合成 API，未调用模型或真实后端，未访问用户安装版/真实项目。粘贴/拖入是构造浏览器事件；本次不声称完成原生系统拖拽或 WebView 验收。临时浏览器和夹具已停止。本地 UI 改动尚未提交/发布，以下 0.1.12 正式发行证据保持有效。

## 2026-09-10 收尾：0.1.12 已公开发布并完成官网同步

以下为最终通过的发行结果，取代本文件后续开发记录中的“待发布/待 CI”状态。桌面源码 c039c7b8a3d43898d48ccedb469cbd79b68005ad；正式构建 v0.1.12-c039c7b8a3d4-10156362780。此前失败/取消的候选没有作为本次正式包发布，历史失败记录保留。

| 检查 | 最终结果 |
| --- | --- |
| Windows CI | [34485419970](https://github.com/rivloom/rivloom-desktop/actions/runs/34485419970)，通过 |
| 官方引擎检查 | [34485420033](https://github.com/rivloom/rivloom-desktop/actions/runs/34485420033)，通过 |
| 发现传输回归 | [34485420034](https://github.com/rivloom/rivloom-desktop/actions/runs/34485420034)，通过 |
| Windows 构建与发布 | [34485688338](https://github.com/rivloom/rivloom-desktop/actions/runs/34485688338)，原生/隔离安装验收、Release、R2、签名更新全部通过 |
| 官网源码与 CI | 7daee76d1d560fcaafad1eb44557e373a01ec9c9；[34489235562](https://github.com/rivloom/rivloom-website/actions/runs/34489235562)，通过 |
| Cloudflare Pages | 42357b69-cee7-44ba-a338-9bf0ad3e36b3，成功 |

另行匿名完整下载 82,373,990 字节，SHA-256 为 f33c9157367de95e1143244373792f1edd1ed7b3ab46ee8404c78b3b3c5f46cf。验证原 0.1.11 更新公钥继续有效，0.1.12 说明、版本与安装包绑定，签名/哈希、不可变版本/latest、no-store 指针与公开校验文件一致，共 10 项通过。证据：[公开包验证](../.data/verification/release-0.1.12-20260910/public-verification.json)。未在用户机器上运行安装器。Windows Authenticode 发布者签名仍未具备，应用内更新签名是独立机制。

官网 41 项测试、18 页构建、670 本地引用与 28 片段链接通过；正式域 23 项检查通过，覆盖 16 路由、3 份与本地构建逐字节一致的静态资源及 0.1.12 下载信息。报告位于官网仓库 .data/verification/continuity-release-20260910/live-report.json。中英文指南/更新日志/隐私页的 12 组桌面与手机 UI 检查已通过；最终中英文下载页在 1280 像素下版本、哈希、布局和控制台检查通过。补查下载页手机视口时浏览器尺寸覆盖未生效，实际仍为 1280，故没有将其计为手机通过。

本轮夹具、预览服务与临时测试页已关闭。功能测试及剩余边界见下一节；本次文档收尾不更改安装包、不重新发布，也不恢复暂停的真实模型/物理机测试。
