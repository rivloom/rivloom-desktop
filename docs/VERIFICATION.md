...
随后浏览器连接恢复，补充界面检查通过：真实界面恢复两份已上传附件草稿，发送后草稿清空且消息进入原会话队列，取消后移除，发送后附件仍可预览；420 像素下图片完整显示、表格/代码/队列和输入区无横向溢出，未出现控制台错误。语言切换检查发现代码块被 memo 缓存而未即时翻译按钮，已订阅语言变化；同一条代码消息不刷新页面即能中英切换，旧语言按钮不再残留。代码原始文本和高亮节点均核对，生产构建与 9 项渲染检查再次通过。官网中英文指南、更新日志和隐私页在 1280 与 390 像素下共 12 组检查通过，无溢出或控制台错误，截图已核对。文件拖入与媒体实际编码播放仍未新增浏览器验收。

首次候选的全部三类云端 CI 通过，打包前校验发现新增 Markdown 依赖尚未加入许可证清单，故未生成或发布安装包。已按锁文件重新采集 234 个版本与 239 份原始声明，没有接受缺失原文或更改发行门槛。补齐后，本机执行与云端相同的完整 runtime 校验通过：201 个 npm 运行时包、316 个 Rust 依赖和 923 份声明文件，Node/OpenCode 二进制哈希与 Cargo 原文一致；报告 .data/verification/continuity-20260910/runtime-after-notices.json。正式发布改由包含声明补齐的后续提交重新进行。
