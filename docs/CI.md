...
> 以下带日期的发行、准备和“未发布”段落为历史记录，不覆盖上述当前状态。

**2026-09-25 · 0.1.21 正式发布与收尾完成。** Windows / Linux x64 同一源码 [3e6e44ea0d0de3bf408d3de0a9c0878890a04bf9](https://github.com/rivloom/rivloom-desktop/commit/3e6e44ea0d0de3bf408d3de0a9c0878890a04bf9)；同提交 CI、Windows 隔离安装/启动/重启/卸载、独立静态解包、公开完整下载及原公钥更新签名通过，Linux 原生与公开 curl/wget、隔离启动/重启/SIGTERM 通过。官网双语下载与说明已实际部署。R2 保留 76、暂缓 0、可删除 0、删除 0 个，检查完成，删除 0 个。核心固定 9b07cf4，ARM64 暂缓。详见 [0.1.21 发行验收](releases/0.1.21-verification.md)。

**2026-09-22 · 0.1.20 正式发布与收尾完成。** Windows x64 与 Linux x64 使用同一源码 [`ef9e4e75ef0d587c137eedfb0fcc4cda106f52b3`](https://github.com/rivloom/rivloom-desktop/commit/ef9e4e75ef0d587c137eedfb0fcc4cda106f52b3)，固定核心仍为 `9b07cf4`。[Windows 构建与发布](https://github.com/rivloom/rivloom-desktop/actions/runs/35712899176)、[Linux 独立发布](https://github.com/rivloom/rivloom-desktop/actions/runs/35713081497)及同提交四组 CI 通过；Windows 云端隔离安装/启动/重启/卸载、独立候选静态核验、公开完整下载与原公钥更新签名，Linux 原生构建及公开包 curl/wget、WSL 启动/重启/SIGTERM 均已按本次产物验证。官网中英文下载页已部署并复核；共 71 个对象：保留 71、暂缓 0、可删除 0、删除 0 个，检查完成，删除 0 个。详见 [0.1.20 发行验收](releases/0.1.20-verification.md) 和 [R2 逐对象审计](releases/0.1.20-r2-retention.md)。ARM64 暂不发布；下方旧版本记录保留为历史。
...
**2026-09-16 · 0.1.17 发行准备：** 用户已明确授权版本加一、提交、推送并沿用既有流程构建发布。本次包含同厂商多个账号与别名、独立 API Key / OAuth、可折叠模型分组、移除接入额度确认框和系统字体清晰度调整。原有本地改动全部保留；历史“未获发行授权”已由本次授权满足。当前正式下载仍为 0.1.16，待同提交云端测试、安装验收、公开下载、签名更新、官网实际部署和 R2 保留检查完成后更新发行结果。内部交接继续仅在本地保存。

**2026-09-16 · 当前本地开发补充，未发布：** 服务矩阵增加 `provider-accounts`，以隔离 Rivloom、官方引擎和本机合成接口验证同厂商两个账号的独立凭据、并发任务、停止/恢复、别名、重启与删除。对应命令为 `node scripts/ci-services.ts provider-accounts`。本地 4 组检查通过，账号单测进入 logic，矩阵和 69 项 CI 自检通过；当前没有这批新增代码的云端运行，下面 0.1.16 发布结果属于此前源码。
...
**2026-09-15 发布结果：** 0.1.15 修复提交 97ab793 的全部 CI、原生构建、隔离安装、公开下载和原密钥签名更新通过。官网实际部署与 R2 保留审计完成，删除 0 个；首次失败、修复及完整证据见维护者本地验证记录。

**2026-09-15 · 0.1.15 发行授权：** 用户已授权版本加一、提交与升级；本次可以推送 main 并进入既有自动发行链。下面开源准备阶段“尚无发行授权”的限制已由本次授权满足，仓库可见性变更仍另行处理。结果以目标提交实际云端运行和维护者本地验证记录为准。

**2026-09-14 开源准备（未推送，09-15 更新）：** 用户确认保留 main CI 成功后自动发布。新增官方仓库/main/版本标签来源约束、非 main 候选拒绝、R2 inventory 的官方 main 限制，以及 3 项工作流策略自检；外部 PR 测试保持只读且无部署凭据。runtime 新增 LICENSE/NOTICE 原文及哈希门禁。414 项逻辑、68 项 CI 自检、独立源码安装/构建通过；当前代码尚无新的云端运行。桌面继续私有；用户已确认在账号内撤销分享，当前引用已清理，原仓库、历史和 CI/CD 保留，详见维护者本地开源准备记录。此轮推送 main 会触发既有发布链，不能未经发行授权直接推送。

**2026-09-14 开发补充（未发布）：** 服务检查增加 `knowledge`，使用三个隔离 Rivloom 服务、官方 OpenCode 与本机模拟模型验证共享 Skills/Wiki。知识库 10 项单测进入 logic，覆盖与 services 矩阵同步；本地通过结果见维护者本地验证记录。本次没有云端运行、安装器验证或发布，不能沿用下方 0.1.14 发布结果证明这些新增代码已发行。

**2026-09-14 发布结果：** 0.1.14 同提交全部 CI、原生构建、隔离安装、公开下载与签名更新通过；官网实际部署和 R2 只读清单/保留审计已完成。具体链接与检查范围见维护者本地验证记录。

**2026-09-14：** 新增手动触发的只读 R2 inventory 工作流，复用既有下载存储凭据与 `public-rivloom-download` 互斥组，列举完整对象并保存报告，供发布执行者逐项核对保留策略。它没有删除功能，也不会在发布后自动运行；本次实际检查结果另见维护者本地验证记录。

**2026-09-11 补充：** 0.1.13 的 CI、安装验收、公开下载、签名更新和官网部署均已完成，以下早期版本段落保留为历史。以后每次正式发布的收尾必须执行 [R2 保留与清理检查](R2-RETENTION.md)：保留最近 3 个正式版本、近 30 天发布的包及当前下载/更新引用，按并集判断。现有 CI 尚未自动清理旧包；由发布执行者完成并记录保留、删除或暂缓结果，不能把规则记载等同自动化上线。

更新日期：2026-09-08。流水线使用 Rivloom 0.1.4 正式身份并发布普通 GitHub Release，实施范围见 发行计划（维护者本地记录）。官网文件同步已启用并完成真实发布验证，签名与 updater 尚未实施。此前 Preview 构建与发布结果保留为历史，不能用本地构建或官网 CI 代替桌面云端结果。
...
随后推送 `0f1c697`，三份 Windows CI、Rust 测试、原生打包与前后 runtime 校验通过；[安装检查 34207169500](https://github.com/rivloom/rivloom-desktop/actions/runs/34207169500) 因仍要求旧符号图片而失败，Release 与官网同步被阻止。当前界面实际引用完整字标与派生 favicon，旧符号图由 Vite 正常移除。检查已改为核对这两张在用图片的唯一性、源文件哈希和产物内引用，仍拒绝缺图、改图、重复、未引用图片与错误主题色；新增回归后 CI 自检 57/57。基础构建现在先运行只读 `ci-ui-assets.ts`，与安装验收共用校验函数，在原生编译前发现资源不一致；完整安装验收仍单独执行。
...
纯 mDNS 的历史最新全量基线是 **154/155**，本批专项再次失败，UDP 路径通过。两者始终分开展示。这里的网络 job 在一台 Windows 主机上的真实网络栈运行；它不能证明两台真实设备、防火墙安装体验或目标 LAN 都已通过。云环境测试失败应保留真实失败；无法提供双机条件时，双机验收仍为未覆盖。发行说明必须列出未解决问题，不能用其他 job 通过替代。维护者本地验证记录
