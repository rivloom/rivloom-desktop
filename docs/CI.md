...
更新日期：2026-09-05。本文记录阶段 B 已编写的工作流与本地验证，**尚未推送或在 GitHub Actions 云 runner 执行**。候选构建已配置门槛，原生安装、签名、公开上传和 updater 仍按 [实施计划](plans/2026-09-05-website-ci-cd-and-updates.md) 与 [RELEASING](RELEASING.md) 分别验收；基础 CI 通过不代表这些能力完成。
...
Rust pin 已包含 2026-09-03 官方补丁对 1.98.0 vtable 误编译的修复；随该工具链发布的 Cargo CLI 版本由 bootstrap 的 `CFG_RELEASE` 决定，不能拿 Cargo crate 的 `0.99.0` 版本推算。此次只核对官方发布与源码，本地未安装/执行 1.98.1 原生构建。[Rust 1.98.1 公告](https://blog.rust-lang.org/2026/09/03/Rust-1.98.1/)、[Cargo 版本逻辑](https://github.com/rust-lang/cargo/blob/797e8a9bca276c1c9f9f738d2a20f484fa4eea9d/src/cargo/version.rs)、[Rust bootstrap](https://github.com/rust-lang/rust/blob/1.98.1/src/bootstrap/src/core/build_steps/tool.rs)
...
Tauri 仅在本次 CLI 合并生成的配置，把已执行的 `beforeBuildCommand` 和 `beforeBundleCommand` 置空，防止门槛之后再次准备/改动 runtime。构建显式使用固定目标、`--locked`、`--no-sign`，并关闭 updater artifacts；源配置、正式 app 版本和原安装数据不被这份临时配置修改。参数已对照锁定的 Tauri CLI help/schema 与[官方 CLI 文档](https://v2.tauri.app/reference/cli/)，本地没有因此运行一次 NSIS 构建。
...
候选 job 不把基础测试或纯 mDNS 的已知失败改写成通过。它检查的是源码/版本、runtime/许可和候选字节的构建门槛；原生安装、NSIS 内容独立解包、更新与物理机验收仍需另行完成。候选 helper 的本地测试使用合成 PE 头验证拒绝逻辑和字节记录，不把该测试文件当作安装器。
