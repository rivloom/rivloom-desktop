...
**2026-09-15 发布结果：** 0.1.15 修复提交 97ab793 的全部 CI、原生构建、隔离安装、公开下载和原密钥签名更新通过。官网实际部署与 R2 保留审计完成，删除 0 个；首次失败、修复及完整证据见 VERIFICATION 顶部。

**2026-09-15 · 0.1.15 发行授权：** 用户已授权版本加一、提交与升级；本次可以推送 main 并进入既有自动发行链。下面开源准备阶段“尚无发行授权”的限制已由本次授权满足，仓库可见性变更仍另行处理。结果以目标提交实际云端运行和 VERIFICATION 顶部为准。

**2026-09-14 开源准备（未推送，09-15 更新）：** 用户确认保留 main CI 成功后自动发布。新增官方仓库/main/版本标签来源约束、非 main 候选拒绝、R2 inventory 的官方 main 限制，以及 3 项工作流策略自检；外部 PR 测试保持只读且无部署凭据。runtime 新增 LICENSE/NOTICE 原文及哈希门禁。414 项逻辑、68 项 CI 自检、独立源码安装/构建通过；当前代码尚无新的云端运行。桌面继续私有；用户已确认在账号内撤销分享，当前引用已清理，原仓库、历史和 CI/CD 保留，详见 [开源准备](OPEN-SOURCE-READINESS.md)。此轮推送 main 会触发既有发布链，不能未经发行授权直接推送。

**2026-09-14 开发补充（未发布）：** 服务检查增加 `knowledge`，使用三个隔离 Rivloom 服务、官方 OpenCode 与本机模拟模型验证共享 Skills/Wiki。知识库 10 项单测进入 logic，覆盖与 services 矩阵同步；本地通过结果见 VERIFICATION 顶部。本次没有云端运行、安装器验证或发布，不能沿用下方 0.1.14 发布结果证明这些新增代码已发行。

**2026-09-14 发布结果：** 0.1.14 同提交全部 CI、原生构建、隔离安装、公开下载与签名更新通过；官网实际部署和 R2 只读清单/保留审计已完成。具体链接与检查范围见 VERIFICATION 顶部。

**2026-09-14：** 新增手动触发的只读 R2 inventory 工作流，复用既有下载存储凭据与 `public-rivloom-download` 互斥组，列举完整对象并保存报告，供发布执行者逐项核对保留策略。它没有删除功能，也不会在发布后自动运行；本次实际检查结果另见 VERIFICATION 顶部。
