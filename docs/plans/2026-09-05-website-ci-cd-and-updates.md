...
**状态：** 2026-09-05 已开始第一轮实施。用户创建的私有 `rivloom/rivloom-website` 已克隆到并列目录；官网首版静态页面、本地预览、桌面 CI 与发行记录基础正在实现及验证。尚无 Cloudflare 正式部署、安装包公开发行或应用内更新完成记录。承接 [RELEASING](../RELEASING.md)、[ADR-0006](../adr/0006-website-distribution-and-safe-updates.md)，不重复定义其安全升级与兼容标准。
...
用户已确认官网域名 `rivloom.com`，并在已登录 Cloudflare 账户中看到该域名。下载/更新子域名尚待设置；官网域名可见不代表 DNS、HTTPS 或部署已通过验收。首版同时面向中国大陆与海外，按用户决定先统一采用 Cloudflare；不预先承诺各地区下载表现。
...
**拟改当前仓库：** `package.json`、测试入口、`scripts/desktop-prepare.ts`、许可检查和 `scripts/desktop-install-smoke.ps1`；拟建 `.github/workflows/ci.yml`、`windows-services.yml`、`lan-regression.yml`、`windows-candidate.yml`。具体拆文件以职责为准，不为每个命令创建一条流水线。
...
5. 把许可生成与验证分开：核对锁定依赖、许可原文、Rust 清单、随包运行时和 README/SECURITY 哈希。当前 Rust 许可清单在 prepare 中为可选，发布检查需改成明确的必需项；缓存命中也验证 Node/OpenCode 二进制哈希。
...
域名、托管方向和首批地区已确认；官网 GitHub 仓库为私有。仍需落实 Cloudflare 仓库连接/部署、下载存储、正式发行签名与密钥保管方式，并核对 GitHub 分支/环境保护能力。现有本机预览、其他未提交规划和正式客户端数据均保留。
