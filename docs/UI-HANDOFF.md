...
更新：2026-09-09。项目：`C:\project\rivloom-opencode`。

**并发设置：** 本机发起的任务不设固定上限；其他机器任务默认 3，可修改为 1–10。队列面板直接显示“本机不限 · 其他机器 n 项”及修改按钮，模型与执行页和 Brain 页也可调整。选择器区分保存值、草稿、保存中和失败重试，保存成功后才显示新的生效值；实时更新保留未保存草稿。Brain Worker 显示其他机器任务的配置上限与当前空闲名额，机器详情分别列本机占用与外来占用/上限。10 项队列提醒继续独立保留。

**本轮新增：** 任务图已经接入真实协调器和官方执行。所有新会话先规划；一个步骤保持简洁，多步骤或转交显示可折叠依赖图。卡片按实际执行段显示 Node、状态和代次，依赖为实线、转交为虚线；查询后的同 Node 继续在详情保留记录，不额外堆卡片。原会话汇总队列确认、工具审批、用户问题和结果文件，历史卡片展示对应历史结果。未开始步骤可修改要求、首选 Node 和依赖，编辑版本过期会拒绝覆盖。流程会话去掉占空间的只读输入框，打开时从进度开始阅读，图可独立滚动。

新输入器默认自动规划，提供可点击的 `@ 首选`/`@@ 锁定` 提示、选择器模式按钮和明确目标标签；切换标签或手动增删第二个 `@` 同步实际提交模式。目标按稳定 ID 保存，删除正文中的名称不会暗中撤销已选目标，可用标签的清除按钮恢复自动。资源目录位于“设备与模型 → 模型与执行”，按机器直接列工作目录：本机完整路径、远端登记名称，保留未选目录、扫描中、部分目录、离线等状态；工具与模型可独立展开。用户手动查询表单已移除，AI 执行过程中的资源查询服务继续保留。Brain 页用“可立即执行 n 项”说明当前空闲执行能力，旁注明确并发槽位与 10 项积压提醒的区别。

300 项逻辑、13 项协议、12 项真实服务检查与 60 项原侧栏 UI 回归通过。新生产 UI 的 20 项检查覆盖中英文四档窗口、图和卡片几何、历史结果、队列/工具动作、目标模式、版本冲突和资源页，浏览器错误和外部请求为零。`785a8157` 本地安装包已完成 Release/NSIS、7,662 个运行文件解包和安装脚本核验。最终截图、精确安装包和哈希以[实施记录](plans/2026-09-09-collaborative-task-flow-implementation.md)为准，用户自行安装，物理设备检查继续暂缓。下方 `a152322c` 及“独立交互概念”描述属于前一轮记录。

**当前状态：** 最新本地候选 `a152322c` 已实现成功执行自动完成，去掉人工验收入口；危险操作审批和补充问题保留。机器备注旁增加删除图标，二次弹窗确认后解除配对。栏目合并为“设备与模型”，含“模型与执行”“节点与 Brain”两页，本机执行能力紧邻模型配置，模型操作记录面板移除，连接诊断在功能栏目最下方。机器条显示“队列 n/10”及真实执行中数量；10 为软提醒，达到后在提交方本机确认追加，并发容量在详情单独展示。附件单个 200 MiB、每批最多 5 个；大文件需双方支持新能力，旧版小文件互通保持。取消确认保留草稿，继续提交重用原创建标识。

历史栏沿用 12px 常规标题、40px 行高、已完成行无右侧勾或占位，以及自己浅/别人较深的来源配色。执行状态由原 4px 呼吸点改为 9px 旋转环，机器队列内为 8px，系统减少动画时静止。合并导航后八组中英文布局的可见历史条数不减少。配对机器继续已连接/默认折叠离线分组、63px 两行条目、左下角硬件图标和信息详情；硬件负载取有效 CPU/内存/GPU 百分比最高值，未知/离线/过期不显示为零。备注与删除图标悬停整行/键盘聚焦显示，触屏常显，详情支持悬停转移、点击、Escape 和窄窗口滚动。

265/265 逻辑、12/12 协议、87/87 界面、新增 4 项真实隔离服务、12 项 Node P0 和 6 项协作文件检查通过；实际 200 MiB 上传及重启续传哈希一致。Release/NSIS、7,645 个运行文件解包和安装脚本检查通过。安装包、SHA-256、源码快照及截图见[本轮记录](plans/2026-09-08-desktop-workflow-simplification.md)。供用户自行安装，未公开发布。

第 6 项的任务编排是独立交互概念：任务卡选择 Node，连线表达上游产物与依赖，并行分支完成后汇合；可以逐步演示，同机分支遵守实际并发。明暗外观和 736px/360px 主要交互已检查，尚未接入真实依赖调度。后续需明确连线编辑、失败阻塞和上游变更后的重跑边界，再落地持久工作流与执行逻辑。

上一轮 `b0a75891` 的整体配色、字体、卡片和输入器记录见[视觉美化记录](plans/2026-09-08-desktop-visual-polish.md)。该轮 JavaScript 与当时基线一致的结论仅适用于该候选；之后增加了历史状态展示与本次机器展示/公开计数协商。`95aa0c2c` 中将两种来源都做成透明底的方案已被用户明确反馈替代；深浅区分继续保留，同时避免原来大块深蓝实底的视觉重量。

用户暂缓的实机/性能补测继续暂缓，当前正在讨论其提出的关键交互。整体交接以 [HANDOFF](HANDOFF.md) 顶部为准，既有实际验收证据及剩余项以 [桌面待测记录](DESKTOP-ACCEPTANCE.md) 为准。以下日期、人工验收、0.1.3 docs1、独立 Preview 与旧候选均是历史阶段记录，不能代替本轮行为、安装状态或当前官网下载来源；视觉与 A–D 功能不从头重做。
