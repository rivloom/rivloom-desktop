...
## 2026-09-15 · 首页图片静态展示与导航调整（已部署）

官网首页截图已改为不可点击的普通图片，移除悬停移动及“点击查看完整界面”。首页开源许可徽章与“了解 Rivloom”次要按钮已删除；顶部桌面/手机菜单移除工作方式、使用指南。安装和模型配置等指南继续从页脚及下载页查阅，旧文档地址保留。页头开源代码和页脚 GitHub 入口均在新标签页打开，保留官网当前页，中英文一致。

官网提交 `fc20b6cfa903a3425edddf628cf7da0eed4d1627` 已推送，[Cloudflare Pages 部署](https://dash.cloudflare.com/?to=/cd32cbd5ec257713adb47572ea24f771/pages/view/rivloom-website/cf6a4456-654f-4501-b767-17ee47d049ac)成功。41 项测试、18 页构建、24 组本地布局与 8 组交互检查通过；正式域 26 项页面/资源/下载检查、12 组桌面/手机布局、4 组交互检查通过，无页面/控制台错误。截图点击保持地址且不新增标签；开源入口真实浏览器新建 GitHub 地址标签、opener 为空，目标响应在测试中隔离以独立验证浏览器行为。公开安装包仍为 0.1.15，HEAD 200、89,188,353 字节。

官网 Actions 保持 disabled_manually，本次没有运行；继续私有并由 Pages 构建部署。证据位于两仓库忽略目录 `.data/verification/home-interactions-20260915/`；官网产品/实施约定同步。桌面仅更新交接，以 `[skip ci]` 提交，无新桌面发行或 R2 删除。以下图片可点击、顶部含工作方式/指南的旧描述不再代表当前界面。
