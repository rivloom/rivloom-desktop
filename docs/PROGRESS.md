...
**2026-09-01 产品修正：当前 M3.3 的“每项任务由接受方选择项目/模型并创建 30 分钟准备租约”不是目标产品流程。** 用户要求节点能力可以自动调用或有限调用，并可选择每次确认；目标默认体验是受信范围内按本机预设策略自动调用。现有准备租约仅作为已经实现并验证过的原型保留，暂不继续扩展，后续需要迁移或删除。下一步先实现可复用的本机能力配置和调用策略，再接真实 OpenCode 执行、审批、停止和产物回传。详见 [ADR-0002](adr/0002-configurable-node-invocation-policy.md)。OpenCode 始终只在实际执行节点本机监听 loopback。
...
| M3 自发现节点与 Brain  | 配对物理通过；邀请/准备原型已验证但流程待重做，下一步为策略化自动调用         |
...
1. 不再把现有 30 分钟准备租约升级成默认执行流程；先定义可复用的本机能力配置，以及自动、有限、每次确认三种调用策略。
2. 让受信节点发来的任务匹配本机策略：自动模式直接创建一次本地任务和 OpenCode 会话，有限模式执行范围检查，每次确认模式才显示人工确认。
3. 建立跨设备角色映射和有序事件回传，再依次接审批、停止、差异、产物与验收。
4. 强制结束进程或断开网络，专项确认另一端约 15 秒显示离线、约 30 秒移除；恢复后自动上线并重建通道。
