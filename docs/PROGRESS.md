...
产品方向已修正：M3 不采用“一台主机开启协作空间、另一台作为访客连接”的方案。安装 Rivloom 的实例都是节点，同网段节点自动发现彼此，并可承载一个或多个 Brain。桌面首次启动现已取消工作区/账号表单，通过启动期本机令牌自动建立操作者，直接进入任务界面并发现节点。M3.1 已完成稳定节点身份、DPAPI 私钥保护、mDNS/DNS-SD、LAN UDP 回退、随机挑战签名验证、受限公开端点和桌面节点页。M3.2 的双端配对闭环已通过同机、Release WebView2 及用户的 Win10/Win11 物理双机；认证加密通道和 Brain 目录同步已通过真实双实例、攻击路径自动化和实际 Release WebView2。M3.3 第一切片已增加加密任务邀请、人工接受/拒绝/取消、双方持久状态和断线重试，但尚未绑定本机项目、角色、模型或 OpenCode 执行。新通道与任务邀请尚待物理双机复测。OpenCode 始终只在实际执行节点本机监听 loopback。架构边界见 [ADR-0001](adr/0001-self-discovering-brain-network.md)。
...
| M3 自发现节点与 Brain  | 配对物理通过；加密任务邀请自动化/Release 通过，待物理复测和执行闭环           |
...
- `server/remote-tasks.ts`、`server/index.ts`、`src/node-network.tsx`：单归属 Brain 的任务邀请、人工接受/拒绝/取消、全局任务 ID、幂等键、24 小时期限、持久化与重试；接受后仍不绑定项目、角色、模型或启动 AI。
...
- `.data/verification/desktop-node-network.json`、`.png` 与 `desktop-node-pairing.png`：12 项实际 Release 节点页检查通过；真实第二实例完成同码、单方不授信、双方授信、加密任务邀请、人工接受和撤销。
- 本地检查：TypeScript/生产构建通过；14 项测试通过，其中信任冲突拒绝、mDNS、LAN UDP、配对重放、双方确认、握手/密文攻击、远端任务幂等、持久化、重启与撤销均有验证；`cargo fmt --check`、`cargo clippy -- -D warnings` 通过；生产依赖离线 audit 为 0 个已知漏洞。

安装包：`src-tauri/target/release/bundle/nsis/Rivloom_0.1.0_x64-setup.exe`，71,062,038 字节，SHA-256 `3b627617174f84c8ef9c363b7b93d3455a418d6b3ca4c33b88d69c18ae967589`。这是包含加密任务邀请第一切片的未签名内测包；实际 Release WebView2 和开发机隔离安装/启动/卸载烟雾测试均已在这个最终包上通过。

## 下一次恢复工作时

1. 把当前包安装到 Win10 `192.168.5.18` 与 Win11 `192.168.5.20`；原有信任应保留，加密通道就绪后从一台发起任务邀请，在另一台接受、拒绝或由发起方取消。
2. 重启任一端，确认任务记录保留且通道自动重建；撤销一端信任，确认两边回到未配对并取消仍处于待处理/已接受的邀请。
3. 强制结束进程或断开网络，专项确认另一端约 15 秒显示离线、约 30 秒移除；恢复后自动上线、重建通道并发送等待中的邀请状态。
4. 继续任务 8：增加本机项目能力声明、接受人的项目/额度确认、跨设备角色映射和有期限的执行租约，再接 OpenCode 执行/审批/事件/产物，不开放 loopback 业务接口。
