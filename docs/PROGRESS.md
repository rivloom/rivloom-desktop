...
测试前再次出现单向发现。只读诊断确认 `5.33` 的节点 TCP 端口正常监听，但随包 `%LOCALAPPDATA%\Rivloom\runtime\node.exe` 的 Private TCP/UDP 入站规则为 `Enabled=Yes, Action=Block`；改为 Allow 后，`5.20 → 5.33` TCP 探测及自动发现恢复。当前安装器仍依赖 Windows 首次联网弹窗，没有可靠创建、检测、升级和卸载命名规则。下一次开发先修复这个产品缺口。
...
| M3.1 节点自发现        | 用户确认完成                                                  |
| M3.2 配对与加密连接    | 用户确认完成                                                  |
| M3.3 跨节点任务协作    | 单人双机核心闭环通过；待停止/自动审批/撤销及两真人角色验收    |
...
1. 为随包 `node.exe` 实现仅限 Private/LocalSubnet 的 Rivloom 命名防火墙规则；安装/升级幂等，卸载清理，并在客户端对 Block/缺失规则给出明确诊断。不得打开公网接口或依赖随机端口白名单。
2. 重建并复测 NSIS：安装、启动、双机发现、升级、退出清理和卸载；至少确认没有残留冲突规则。
3. 补测远程停止、“帮我批准 / 允许任何操作”、执行中撤销来源设备信任和重启保持；基础请求批准、补充和验收不再重复设计。
4. 在两位真人实测中明确发起、执行、审批和验收人的跨设备身份映射；不能用同机双实例或前端视角切换代替。
5. 强制结束进程或断开网络，专项确认另一端约 15 秒显示离线、约 30 秒移除；恢复后自动上线、重建通道且不重复执行任务。单边通道丢失的自动回归已通过。
