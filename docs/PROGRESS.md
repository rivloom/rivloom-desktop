...
产品负责人已重新确认：M0、M3.1 和 M3.2 完成；M1 下一步使用已创建的 DeepSeek Key 做真实验证；M2 的正式发行完善暂缓；当前产品开发停在 M3.3。M3.4 的 Brain 定义存在认识差异，讨论清楚前暂停设计和实现。

M1 的模型设置产品功能和无 Key 验证已经完成。当前 Windows 客户端包含模型设置页、DeepSeek 工作区凭据管理、真实连接测试、默认模型、任务模型选择和无密钥审计记录；官方 OpenCode 仍是唯一执行引擎。

用户已经创建 DeepSeek Key，但尚未在 Rivloom 本机完成真实验证，因此还没有 DeepSeek 成功回复或 DeepSeek 编程任务报告。Key 不应提供给聊天或仓库。所有不需要真实 Key 的工作已经完成：使用测试字符串通过官方 OpenCode `auth.set` / `auth.remove` 和 `provider.list` 验证凭据生命周期；业务数据库与接口响应未出现 Key；真实 Release WebView2 页面和新安装包均已验证。
...
| M3.1 节点自发现        | 用户确认完成                                                                  |
| M3.2 配对与加密连接    | 用户确认完成                                                                  |
| M3.3 跨节点任务协作    | 当前里程碑；自动调用和真实执行已完成第一段，远程控制/产物待对齐后继续         |
