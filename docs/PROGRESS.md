...
产品方向已修正：M3 不采用“一台主机开启协作空间、另一台作为访客连接”的方案。安装 Rivloom 的实例都是节点，同网段节点自动发现彼此，并可承载一个或多个 Brain。桌面首次启动现已取消工作区/账号表单，通过启动期本机令牌自动建立操作者，直接进入任务界面并发现节点。M3.1 已完成节点身份与发现，M3.2 配对闭环已通过用户的 Win10/Win11 物理双机，认证加密通道和 Brain 目录已通过真实双实例、攻击路径自动化和实际 Release WebView2。M3.3 已完成任务邀请切片和本机执行准备切片：接受方只在自己的设备选择可信项目/模型，用 30 分钟准备租约暂时保留项目；发起方只收到状态与期限。角色映射、可消费的执行租约、OpenCode 执行、审批和产物仍未跨节点开放。新通道、邀请与准备租约尚待物理双机复测。OpenCode 始终只在实际执行节点本机监听 loopback。架构边界见 [ADR-0001](adr/0001-self-discovering-brain-network.md)。
...
| M3 自发现节点与 Brain  | 配对物理通过；邀请/本机准备自动化与 Release 通过，待物理复测和执行闭环        |
...
- `server/node-channel.ts`、`server/node-network.ts`：Ed25519 签名临时 X25519 握手、HKDF 方向密钥、AES-256-GCM、严格序号/时间窗、10 分钟轮换和撤销断链；当前开放最小 Brain 目录和任务邀请控制消息。
- `server/remote-tasks.ts`、`server/index.ts`、`src/node-network.tsx`：单归属 Brain 的任务邀请、人工接受/拒绝/取消，以及接受方本机项目/模型选择。30 分钟准备租约会阻止普通任务从该项目启动，可撤销/过期/重建；节点消息不含本机项目或模型标识，也不会启动 AI。
...
- `.data/verification/desktop-node-network.json`、`.png` 与 `desktop-node-pairing.png`：16 项实际 Release 节点页检查通过；真实第二实例完成加密邀请、人工接受、本机项目/模型准备、隐私边界、普通任务拦截、准备撤销和设备撤销。
- 本地检查：TypeScript/生产构建通过；14 项测试通过，其中握手/密文攻击、远端任务幂等、第一切片记录迁移、准备租约、自动过期、持久化、重启与撤销均有验证；`cargo fmt --check`、`cargo clippy -- -D warnings` 通过；生产依赖离线 audit 为 0 个已知漏洞。

安装包：`src-tauri/target/release/bundle/nsis/Rivloom_0.1.0_x64-setup.exe`，71,024,519 字节，SHA-256 `0f8a150c2e1f4f5cb20c096d5c99de8be66af61a5bf26dfe599a484944a0ed3c`。这是包含本机执行准备租约的未签名内测包；实际 Release WebView2 和安装/启动/卸载烟雾测试均已在这个最终包上通过。

## 下一次恢复工作时

1. 把当前包安装到 Win10 `192.168.5.18` 与 Win11 `192.168.5.20`；从一台发起邀请，另一台接受并选择本机可信项目/模型，确认发起方只显示“执行准备就绪”和期限。
2. 在接受方撤销准备，确认项目立即释放；重新准备后重启任一端，确认记录与租约保留、通道自动重建。撤销设备信任应取消邀请并释放项目。
3. 强制结束进程或断开网络，专项确认另一端约 15 秒显示离线、约 30 秒移除；恢复后自动上线、重建通道并发送等待中的邀请/准备状态。
4. 继续任务 8：实现跨设备人员角色映射、将准备租约升级成只能消费一次的执行租约，再建立本地任务/OpenCode 会话和有序事件回传；之后接审批、停止、差异与产物。
