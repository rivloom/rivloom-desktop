...
**2026-09-01 的 M3.3 产品修正已经实现第一段。** 本机所有者一次配置项目、模型和调用方式：自动调用是界面默认，也可限制到指定受信节点或改为每项确认。匹配的加密协作任务只创建一个本机业务任务和官方 OpenCode 会话；归属 Brain 按单调序号收到状态和最终摘要，但收不到本机项目/模型/任务 ID 或凭据。旧版 30 分钟准备记录会迁移为不可自动执行的历史记录，旧界面已删除。实际 Release WebView2 已让真实第二节点发起任务，由随包 OpenCode 在隔离项目文件夹生成并核对 `RESULT.txt`。跨设备审批、补充要求、停止、差异/产物和验收仍待下一切片。详见 [ADR-0002](adr/0002-configurable-node-invocation-policy.md)。OpenCode 始终只在实际执行节点本机监听 loopback。
