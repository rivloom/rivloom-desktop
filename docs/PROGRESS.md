...
| M3.1 节点自发现        | 用户确认完成                                                  |
| M3.2 配对与加密连接    | 用户确认完成                                                  |
| M3.3 跨节点任务协作    | 功能闭环已实现；等待物理双机上的角色、补充、结果和验收确认    |
...
1. 用 Win10 `192.168.5.18` 与 Win11 `192.168.5.20` 物理复测受信任务自动接收、三种 AI 审批、远程批准/回答/停止、补充要求、官方差异提示、最终验收和撤销信任停止任务。
2. 在两位真人实测中明确发起、执行、审批和验收人的跨设备身份映射；不能用同机双实例或前端视角切换代替。
3. 使用已配置且可用的模型重新跑新增的真实同会话补充与最终验收夹具；免费模型无响应不能算产品通过或失败。
4. 强制结束进程或断开网络，专项确认另一端约 15 秒显示离线、约 30 秒移除；恢复后自动上线、重建通道且不重复执行任务。单边通道丢失的自动回归已通过。
5. 正式发给更多内测者前，复测当前 NSIS 安装/启动/卸载，并完成干净 Windows 虚拟机矩阵、代码签名和发布安全审查。
