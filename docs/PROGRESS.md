...
产品方向已修正：M3 不采用“一台主机开启协作空间、另一台作为访客连接”的方案。安装 Rivloom 的实例都是节点，同网段节点自动发现彼此，并可承载一个或多个 Brain。Brain 以任务归属和委派协作；OpenCode 仍只在实际执行节点本机监听 loopback。架构边界见 [ADR-0001](adr/0001-self-discovering-brain-network.md)。
...
| M3 自发现节点与 Brain  | 下一里程碑；方向已确认，节点发现、设备信任和跨 Brain 委派尚未实现             |
...
- 本地检查：TypeScript/生产构建通过；6 项安全测试通过；`cargo fmt --check`、`cargo clippy -- -D warnings` 通过；生产依赖离线 audit 为 0 个已知漏洞。

安装包：`src-tauri/target/release/bundle/nsis/Rivloom_0.1.0_x64-setup.exe`，70,998,334 字节，SHA-256 `2ec2a07c782e3842f09cac0a32ca17ca286f26dc9d1935db1f95b4eb34256dd3`。这是未签名内测包。
...
4. 若用户暂不提供 Key，直接进入 M3.1：实现稳定节点身份、mDNS/DNS-SD 自发现、附近节点界面和双端配对；随后完成单 Brain 分布式任务与最小跨 Brain 委派。
5. 正式发给更多内测者前，再做干净 Windows 虚拟机安装/升级/卸载、代码签名和发布安全审查。
