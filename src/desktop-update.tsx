...
export function DesktopUpdateProvider({ children }: { children: ReactNode }) {
  ...
  const installationRevision = useRef<number | null>(null);
  const statusRequest = useRef<Promise<DesktopUpdateSnapshot> | null>(null);

  const readSnapshot = () => {
    if (!statusRequest.current) {
      const request = desktopUpdateSnapshot();
  ...
  const apply = (value: DesktopUpdateSnapshot) => {
    ...
    if (installationRevision.current !== null && value.revision < installationRevision.current) return;
    ...
    if (installationRevision.current !== null && !updateIsInstalling(value.phase) && value.error) {
      installationRevision.current = null; setVisible(true);
    }
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try { const value = await readSnapshot(); if (!stopped) apply(value); }
      catch { if (!stopped) setUnavailable(true); }
      if (!stopped) timer = setTimeout(() => void poll(), startingInstall || snapshotRef.current && updateIsBusy(snapshotRef.current.phase) ? 700 : 15_000);
    };
    void poll();
    return () => { stopped = true; clearTimeout(timer); };
  }, [snapshot?.phase, startingInstall]);
  ...
  const run = async (operation: () => Promise<DesktopUpdateSnapshot>, optimistic?: DesktopUpdateSnapshot['phase']) => {
    ...
    catch { if (mounted.current) { setRequestError(t('更新未完成，请稍后重试。')); await readSnapshot().then(apply).catch(() => undefined); } }
    finally { inAction.current = false; if (mounted.current) setActionBusy(false); }
  };
  const check = () => {
    setVisible(true);
  ...
  const close = () => {
    if (startingInstall || snapshot && updateIsInstalling(snapshot.phase)) return;
  ...
  const startInstallation = async () => {
    installationRevision.current = (snapshotRef.current?.revision ?? 0) + 1;
    // Remove the HTML modal before invoking the native shutdown/installer hop.
    // Installation progress has no modal focus lock and needs no dismiss click.
    flushSync(() => { setVisible(false); setStartingInstall(true); });
    try {
      const accepted = await installDesktopUpdate();
      apply(accepted);
      return accepted;
    } catch (error) {
      installationRevision.current = null;
      if (mounted.current) setVisible(true);
      throw error;
    } finally { if (mounted.current) setStartingInstall(false); }
  };
  const download = () => void run(async () => {
    const value = await downloadDesktopUpdate(); apply(value);
    return value.phase === 'ready' && !value.error ? startInstallation() : value;
  }, 'downloading');
  const cancel = () => {
  ...
  const installing = startingInstall || !!phase && updateIsInstalling(phase);
  const installMessage = phase === 'stopping' ? t('正在退出后台服务…') : phase === 'backing_up' ? t('正在备份应用数据…') :
    phase === 'installing' ? t('正在启动安装，Rivloom 即将关闭…') : t('正在确认任务状态…');

  return <UpdateContext.Provider value={{ snapshot, check, open: () => setVisible(true), unavailable }}>
    {children}
    {desktop && installing && <section className="desktop-update desktop-update-progress" role="status" aria-live="polite" aria-label={t('软件更新')}>
      <strong>{t('正在更新 Rivloom')}</strong>
      <p className="desktop-update-state"><LoaderCircle size={19} className="spin" />{installMessage}</p>
      {phase === 'backing_up' && snapshot?.backup && <div className="desktop-update-download">
        <p>{t('已备份 {{done}} / {{total}} 个文件', { done: snapshot.backup.files, total: snapshot.backup.totalFiles })}</p>
        <progress aria-label={t('应用数据备份进度')} max={snapshot.backup.totalBytes || 1} value={snapshot.backup.bytes} />
        <small>{byteLabel(snapshot.backup.bytes)} / {byteLabel(snapshot.backup.totalBytes)}</small>
      </div>}
      <p className="desktop-update-note">{t('安装完成后将自动重新打开 Rivloom。')}</p>
    </section>}
    {desktop && visible && !installing && <Modal title={snapshot?.release ? t('Rivloom 有新版本') : t('软件更新')} close={close}
      ...
      <div className="desktop-update" aria-live="polite">
        ...
        {snapshot?.release && phase !== 'downloading' &&
        ...
        <div className="modal-actions desktop-update-actions">
          {phase === 'downloading' ? <><Button onClick={close}>{t('后台下载')}</Button><Button onClick={cancel}>{t('取消下载')}</Button></> :
            snapshot?.release ? <>
              ...
              {phase === 'ready' ? <Button disabled={busy} variant="primary" onClick={() => void run(startInstallation)}><RotateCw size={15} />{t('重试安装')}</Button> :
...
export function DesktopUpdateSettings({ beforeCheck }: { beforeCheck?: () => void }) {
  ...
  return <section className="desktop-update-settings" aria-label={t('软件更新')}>
    <div><strong>{t('软件更新')}</strong><Button disabled={!!snapshot && updateIsBusy(snapshot.phase)} onClick={() => { beforeCheck?.(); check(); }}><RotateCw size={14} />{t('检查更新')}</Button></div>
    ...
    {snapshot?.release && <button type="button" className="text-button" onClick={() => { beforeCheck?.(); open(); }}>{t('查看 {{version}} 更新', { version: snapshot.release.version })}</button>}
