...
export function downloadObjectKey(key: string) {
  requireDownload(
    exact(
      /^(?:releases\/(?:latest\.json|linux\/(?:latest\.json|linux-v[0-9A-Za-z.+_-]{1,200}\/(?:Rivloom_[0-9A-Za-z.+_-]{1,100}_linux_(?:x64|arm64)\.tar\.gz|SHA256SUMS\.txt))|v[0-9A-Za-z.+_-]{1,200}\/(?:Rivloom_[0-9A-Za-z.+_-]{1,100}_x64-setup\.exe|SHA256SUMS\.txt))|updates\/stable\/(?:latest|(?:0|[1-9][0-9]*)\.(?:0|[1-9][0-9]*)\.(?:0|[1-9][0-9]*))\.json)$/,
      key,
    ),
