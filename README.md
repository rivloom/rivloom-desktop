...
**0.1.12：** 工作流会话支持完成后继续提出要求，也可以在执行中排队，按轮次保留完整需求、回答、结果与文件。新增 Markdown 表格和代码高亮/复制、跨重启草稿、截图粘贴/文件拖入及图片、文本、音视频预览。见[连续会话说明](docs/CONVERSATION-CONTINUITY.md)。0.1.11 已公开验证，本版按正式 CI、隔离安装与公开签名核验流程发布。以下带日期的记录保留供追溯。
