...
**0.1.7：** 修复下载完成后安装交接仍占用更新弹窗的问题。安装自动转入非模态进度，分别显示任务检查、后台退出、文件备份与启动安装；无需手动关闭弹窗。0.1.6 已公开发行，0.1.7 按正式 CI 与安装验收流程发布。旧客户端发起本次升级时仍使用旧更新器，修复在安装 0.1.7 后生效。见[应用内更新说明](docs/DESKTOP-UPDATES.md)。以下带日期的旧候选记录仅供历史追溯。
