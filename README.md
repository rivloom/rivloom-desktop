...
自动发现不等于自动信任。M3.2 已增加两端短码/指纹核对、双方分别确认、持久信任、取消和撤销；旧配对请求重放、单方确认和已撤销关系都不能建立信任。真实同机双实例、Release WebView2 及用户的 Win10/Win11 物理双机均已验证配对闭环。受信节点现在会用签名的临时 X25519 密钥建立双向认证通道，经 HKDF 派生方向密钥，以 AES-256-GCM、严格消息序号和时间窗保护消息；当前只在通道内同步最小 Brain 目录和能力，已自动验证篡改、重放、过期、重启重建和撤销断链，并通过实际 Release WebView2。React 业务服务、OpenCode、任务、项目和模型接口仍只监听 `127.0.0.1`，没有直接暴露到局域网。**尚未由两个真人完成跨设备任务协作，也尚未实现加密任务传输和任务委派**。详见 [ADR-0001](docs/adr/0001-self-discovering-brain-network.md)。
