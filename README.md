...
自动发现不等于自动信任。M3.2 的第一切片已增加两端短码/指纹核对、双方分别确认、持久信任、取消和撤销；旧配对请求重放、单方确认和已撤销关系都不能建立信任。该流程已通过真实同机双实例和 Release WebView2 验证，仍待 Win10/Win11 两台物理设备复测。局域网端点只交换签名公开身份与配对控制消息，不包含业务接口；React 业务服务和 OpenCode 仍只监听 `127.0.0.1`。**尚未由两个真人完成跨设备任务协作，也尚未实现加密任务传输和任务委派**。详见 [ADR-0001](docs/adr/0001-self-discovering-brain-network.md)。
