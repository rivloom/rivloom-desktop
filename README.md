...
**0.1.6：** 历史会话已恢复删除入口，删除后进入回收站并保留 3 个日历月，支持恢复、永久删除和清空。历史列表按工作目录分组。见[会话历史说明](docs/CONVERSATION-HISTORY.md)。0.1.5 已公开发行并启用应用内签名更新，本轮 0.1.6 按正式 CI 与安装验收流程发布；以下带日期的旧候选记录仅供历史追溯。
