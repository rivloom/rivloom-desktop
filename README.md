...
本次源码为 **0.1.17**，新增同厂商多账号别名、可折叠模型分组和更清晰的系统字体，详见 [版本说明](docs/releases/0.1.17.md)。发行准备期间当前正式版为 **0.1.16**，新增按服务商分组搜索的模型菜单，并改进账号登录、API Key 和自定义服务入口；保留共享 Skills、渐进式 Wiki 记忆、会话搜索、跨设备协作、审批、成果取回和签名更新。可下载版本以[官网下载页](https://rivloom.com/download/)为准。
