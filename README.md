...
**0.1.8：** 侧栏工作目录支持自定义别名，默认显示文件夹名称；会话向右缩进，移除整体历史标题和总数，缩小并上移本机在线区域，窗口标题统一为 Rivloom。0.1.7 已公开发行，0.1.8 按正式 CI 与安装验收流程发布，沿用 0.1.7 的自动安装交接和备份进度。见[会话与目录说明](docs/CONVERSATION-HISTORY.md)。以下带日期的旧候选记录仅供历史追溯。
