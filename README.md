...
**0.1.10：** 目录、会话和文件均支持右键菜单与更多按钮：目录别名/复制完整路径/打开文件夹，会话重命名/置顶/回收站，文件打开/定位/另存为/复制路径。名称与置顶按当前用户持久保存，置顶位于所属目录最前。0.1.9 已公开验证，本版按正式 CI 与安装验收流程发布。见[右键菜单](docs/CONTEXT-MENUS.md)和[会话历史](docs/CONVERSATION-HISTORY.md)。以下带日期的旧候选记录供历史追溯。
