...
**0.1.9：** 新版工作区正常启动后自动删除所有已完成升级的备份，包括旧版遗留与中断副本；失败启动保留备份，文件占用自动重试。会话删除按钮在悬停或键盘焦点进入时显示，触屏保留可见入口。0.1.8 已公开发行，本版按正式 CI 与安装验收流程发布。见[应用内更新](docs/DESKTOP-UPDATES.md)和[会话与目录](docs/CONVERSATION-HISTORY.md)。以下带日期的旧候选记录仅供历史追溯。
