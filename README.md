...
**0.1.11：** 用户消息直接显示正文，去掉“你 / 复制”标题行；消息、执行结果和工具输出的复制改为悬停显示的小图标，保留键盘与触屏操作。正文明确允许选择并使用清晰的选中高亮，保留原始换行和缩进。0.1.10 已公开验证，本版按正式 CI 与安装验收流程发布。见[消息复制说明](docs/MESSAGE-COPY.md)、[右键菜单](docs/CONTEXT-MENUS.md)与[会话历史](docs/CONVERSATION-HISTORY.md)。以下带日期的旧候选记录供历史追溯。
